...
loadjs.ready("head", function() {
	...
	fgaji_karyawan_sdadd.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($gaji_karyawan_sd_add->datetime->Required) { ?>
				elm = this.getElements("x" + infix + "_datetime");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gaji_karyawan_sd_add->datetime->caption(), $gaji_karyawan_sd_add->datetime->RequiredErrorMessage)) ?>");
			<?php } ?>
			<?php if ($gaji_karyawan_sd_add->by->Required) { ?>
				elm = this.getElements("x" + infix + "_by");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gaji_karyawan_sd_add->by->caption(), $gaji_karyawan_sd_add->by->RequiredErrorMessage)) ?>");
			<?php } ?>
			<?php if ($gaji_karyawan_sd_add->jp->Required) { ?>
				elm = this.getElements("x" + infix + "_jp");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gaji_karyawan_sd_add->jp->caption(), $gaji_karyawan_sd_add->jp->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_jp");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($gaji_karyawan_sd_add->jp->errorMessage()) ?>");
	...
	fgaji_karyawan_sdadd.lists["x_by"] = <?php echo $gaji_karyawan_sd_add->by->Lookup->toClientList($gaji_karyawan_sd_add) ?>;
	fgaji_karyawan_sdadd.lists["x_by"].options = <?php echo JsonEncode($gaji_karyawan_sd_add->by->lookupOptions()) ?>;
...
<?php if ($gaji_karyawan_sd_add->jp->Visible) { // jp ?>
	<div id="r_jp" class="form-group row">
		<label id="elh_gaji_karyawan_sd_jp" for="x_jp" class="<?php echo $gaji_karyawan_sd_add->LeftColumnClass ?>"><?php echo $gaji_karyawan_sd_add->jp->caption() ?><?php echo $gaji_karyawan_sd_add->jp->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $gaji_karyawan_sd_add->RightColumnClass ?>"><div <?php echo $gaji_karyawan_sd_add->jp->cellAttributes() ?>>
<span id="el_gaji_karyawan_sd_jp">
<input type="text" data-table="gaji_karyawan_sd" data-field="x_jp" name="x_jp" id="x_jp" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($gaji_karyawan_sd_add->jp->getPlaceHolder()) ?>" value="<?php echo $gaji_karyawan_sd_add->jp->EditValue ?>"<?php echo $gaji_karyawan_sd_add->jp->editAttributes() ?>>
</span>
<?php echo $gaji_karyawan_sd_add->jp->CustomMsg ?></div></div>
	</div>
<?php } ?>
