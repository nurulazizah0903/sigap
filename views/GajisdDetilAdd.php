...
var fgajisd_detiladd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fgajisd_detiladd = new ew.Form("fgajisd_detiladd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fgajisd_detiladd;

    // Add fields
    var fields = currentTable.fields;
    fgajisd_detiladd.addFields([
    ...
    // Form_CustomValidate
    fgajisd_detiladd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fgajisd_detiladd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fgajisd_detiladd.lists.pegawai_id = <?= $Page->pegawai_id->toClientList($Page) ?>;
    fgajisd_detiladd.lists.jabatan_id = <?= $Page->jabatan_id->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fgajisd_detiladd" id="fgajisd_detiladd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->pid->Visible) { // pid ?>
    <div id="r_pid"<?= $Page->pid->rowAttributes() ?>>
        <label id="elh_gajisd_detil_pid" for="x_pid" class="<?= $Page->LeftColumnClass ?>"><?= $Page->pid->caption() ?><?= $Page->pid->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="<?= $Page->pid->getInputTextType() ?>" name="x_pid" id="x_pid" data-table="gajisd_detil" data-field="x_pid" value="<?= $Page->pid->EditValue ?>" size="30" placeholder="<?= HtmlEncode($Page->pid->getPlaceHolder()) ?>"<?= $Page->pid->editAttributes() ?> aria-describedby="x_pid_help">
...
<?php $Page->pegawai_id->EditAttrs->prepend("onchange", "ew.autoFill(this);"); ?>
    <select
        ...
        data-select2-id="fgajisd_detiladd_x_pegawai_id"
...
loadjs.ready("fgajisd_detiladd", function() {
    var options = { name: "x_pegawai_id", selectId: "fgajisd_detiladd_x_pegawai_id" };
    if (fgajisd_detiladd.lists.pegawai_id.lookupOptions.length) {
        options.data = { id: "x_pegawai_id", form: "fgajisd_detiladd" };
    } else {
        options.ajax = { id: "x_pegawai_id", form: "fgajisd_detiladd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready("fgajisd_detiladd", function() {
    fgajisd_detiladd.createAutoSuggest(Object.assign({"id":"x_jabatan_id","forceSelect":false}, ew.vars.tables.gajisd_detil.fields.jabatan_id.autoSuggestOptions));
});
</script>
