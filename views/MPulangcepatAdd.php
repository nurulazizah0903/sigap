...
// Page object
$MPulangcepatAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { m_pulangcepat: currentTable } });
var currentForm, currentPageID;
var fm_pulangcepatadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fm_pulangcepatadd = new ew.Form("fm_pulangcepatadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fm_pulangcepatadd;

    // Add fields
    var fields = currentTable.fields;
    fm_pulangcepatadd.addFields([
    ...
    // Form_CustomValidate
    fm_pulangcepatadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fm_pulangcepatadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fm_pulangcepatadd.lists.jenjang_id = <?= $Page->jenjang_id->toClientList($Page) ?>;
    fm_pulangcepatadd.lists.jabatan_id = <?= $Page->jabatan_id->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fm_pulangcepatadd" id="fm_pulangcepatadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_m_pulangcepat_jenjang_id">
    <select
        ...
        data-select2-id="fm_pulangcepatadd_x_jenjang_id"
...
loadjs.ready("fm_pulangcepatadd", function() {
    var options = { name: "x_jenjang_id", selectId: "fm_pulangcepatadd_x_jenjang_id" };
    if (fm_pulangcepatadd.lists.jenjang_id.lookupOptions.length) {
        options.data = { id: "x_jenjang_id", form: "fm_pulangcepatadd" };
    } else {
        options.ajax = { id: "x_jenjang_id", form: "fm_pulangcepatadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_m_pulangcepat_jabatan_id">
    <select
        ...
        data-select2-id="fm_pulangcepatadd_x_jabatan_id"
        data-table="m_pulangcepat"
...
<script>
loadjs.ready("fm_pulangcepatadd", function() {
    var options = { name: "x_jabatan_id", selectId: "fm_pulangcepatadd_x_jabatan_id" };
    if (fm_pulangcepatadd.lists.jabatan_id.lookupOptions.length) {
        options.data = { id: "x_jabatan_id", form: "fm_pulangcepatadd" };
    } else {
        options.ajax = { id: "x_jabatan_id", form: "fm_pulangcepatadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
