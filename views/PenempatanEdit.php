...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpenempatanedit = new ew.Form("fpenempatanedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fpenempatanedit;

    // Add fields
    var fields = currentTable.fields;
    fpenempatanedit.addFields([
    ...
    // Form_CustomValidate
    fpenempatanedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpenempatanedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpenempatanedit.lists.pegawai = <?= $Page->pegawai->toClientList($Page) ?>;
    fpenempatanedit.lists.project = <?= $Page->project->toClientList($Page) ?>;
    fpenempatanedit.lists.jabatan = <?= $Page->jabatan->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpenempatanedit" id="fpenempatanedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_penempatan_pegawai">
    <select
        ...
        data-select2-id="fpenempatanedit_x_pegawai"
...
loadjs.ready("fpenempatanedit", function() {
    var options = { name: "x_pegawai", selectId: "fpenempatanedit_x_pegawai" };
    if (fpenempatanedit.lists.pegawai.lookupOptions.length) {
        options.data = { id: "x_pegawai", form: "fpenempatanedit" };
    } else {
        options.ajax = { id: "x_pegawai", form: "fpenempatanedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_penempatan_project">
    <select
        ...
        data-select2-id="fpenempatanedit_x_project"
        data-table="penempatan"
...
<script>
loadjs.ready("fpenempatanedit", function() {
    var options = { name: "x_project", selectId: "fpenempatanedit_x_project" };
    if (fpenempatanedit.lists.project.lookupOptions.length) {
        options.data = { id: "x_project", form: "fpenempatanedit" };
    } else {
        options.ajax = { id: "x_project", form: "fpenempatanedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_penempatan_jabatan">
    <select
        ...
        data-select2-id="fpenempatanedit_x_jabatan"
        data-table="penempatan"
...
<script>
loadjs.ready("fpenempatanedit", function() {
    var options = { name: "x_jabatan", selectId: "fpenempatanedit_x_jabatan" };
    if (fpenempatanedit.lists.jabatan.lookupOptions.length) {
        options.data = { id: "x_jabatan", form: "fpenempatanedit" };
    } else {
        options.ajax = { id: "x_jabatan", form: "fpenempatanedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fpenempatanedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpenempatanedit", "x_tgl_mulai", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fpenempatanedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpenempatanedit", "x_tgl_akhir", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="penempatan" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
