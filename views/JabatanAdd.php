...
loadjs.ready(["wrapper", "head"], function () {
    ...
    fjabatanadd.lists.jenjang = <?= $Page->jenjang->toClientList($Page) ?>;
    fjabatanadd.lists.c_by = <?= $Page->c_by->toClientList($Page) ?>;
    fjabatanadd.lists.u_by = <?= $Page->u_by->toClientList($Page) ?>;
...
<span id="el_jabatan_jenjang">
    <select
        ...
        data-select2-id="fjabatanadd_x_jenjang"
        data-table="jabatan"
...
<script>
loadjs.ready("fjabatanadd", function() {
    var options = { name: "x_jenjang", selectId: "fjabatanadd_x_jenjang" };
    if (fjabatanadd.lists.jenjang.lookupOptions.length) {
        options.data = { id: "x_jenjang", form: "fjabatanadd" };
    } else {
        options.ajax = { id: "x_jenjang", form: "fjabatanadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fjabatanadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fjabatanadd", "x_c_date", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_jabatan_c_by">
    <select
        ...
        data-select2-id="fjabatanadd_x_c_by"
        data-table="jabatan"
        data-field="x_c_by"
...
<script>
loadjs.ready("fjabatanadd", function() {
    var options = { name: "x_c_by", selectId: "fjabatanadd_x_c_by" };
    if (fjabatanadd.lists.c_by.lookupOptions.length) {
        options.data = { id: "x_c_by", form: "fjabatanadd" };
    } else {
        options.ajax = { id: "x_c_by", form: "fjabatanadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("gajitunjangan", explode(",", $Page->getCurrentDetailTable())) && $gajitunjangan->DetailAdd) {
?>
