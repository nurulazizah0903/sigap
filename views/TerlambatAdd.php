...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fterlambatadd = new ew.Form("fterlambatadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fterlambatadd;

    // Add fields
    var fields = currentTable.fields;
    fterlambatadd.addFields([
    ...
    // Form_CustomValidate
    fterlambatadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fterlambatadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fterlambatadd.lists.jenjang_id = <?= $Page->jenjang_id->toClientList($Page) ?>;
    fterlambatadd.lists.jabatan_id = <?= $Page->jabatan_id->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fterlambatadd" id="fterlambatadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_terlambat_jenjang_id">
    <select
        ...
        data-select2-id="fterlambatadd_x_jenjang_id"
...
loadjs.ready("fterlambatadd", function() {
    var options = { name: "x_jenjang_id", selectId: "fterlambatadd_x_jenjang_id" };
    if (fterlambatadd.lists.jenjang_id.lookupOptions.length) {
        options.data = { id: "x_jenjang_id", form: "fterlambatadd" };
    } else {
        options.ajax = { id: "x_jenjang_id", form: "fterlambatadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_terlambat_jabatan_id">
    <select
        ...
        data-select2-id="fterlambatadd_x_jabatan_id"
        data-table="terlambat"
...
<script>
loadjs.ready("fterlambatadd", function() {
    var options = { name: "x_jabatan_id", selectId: "fterlambatadd_x_jabatan_id" };
    if (fterlambatadd.lists.jabatan_id.lookupOptions.length) {
        options.data = { id: "x_jabatan_id", form: "fterlambatadd" };
    } else {
        options.ajax = { id: "x_jabatan_id", form: "fterlambatadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
