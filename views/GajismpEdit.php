...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fgajismpedit = new ew.Form("fgajismpedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fgajismpedit;

    // Add fields
    var fields = currentTable.fields;
    fgajismpedit.addFields([
    ...
    // Form_CustomValidate
    fgajismpedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fgajismpedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fgajismpedit.lists.bulan = <?= $Page->bulan->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fgajismpedit" id="fgajismpedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_gajismp_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_gajismp_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="gajismp" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_gajismp_bulan">
    <select
        ...
        data-select2-id="fgajismpedit_x_bulan"
        data-table="gajismp"
        data-field="x_bulan"
...
<script>
loadjs.ready("fgajismpedit", function() {
    var options = { name: "x_bulan", selectId: "fgajismpedit_x_bulan" };
    if (fgajismpedit.lists.bulan.lookupOptions.length) {
        options.data = { id: "x_bulan", form: "fgajismpedit" };
    } else {
        options.ajax = { id: "x_bulan", form: "fgajismpedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("gajismp_detil", explode(",", $Page->getCurrentDetailTable())) && $gajismp_detil->DetailEdit) {
?>
