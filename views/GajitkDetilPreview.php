...
// Page object
$GajitkDetilPreview = &$Page;
?>
<script>
ew.deepAssign(ew.vars, { tables: { gajitk_detil: <?= JsonEncode($Page->toClientVar()) ?> } });
</script>
...
<?php if ($Page->voucher->Visible) { // voucher ?>
    <?php if ($Page->SortUrl($Page->voucher) == "") { ?>
        <th class="<?= $Page->voucher->headerCellClass() ?>"><?= $Page->voucher->caption() ?></th>
    <?php } else { ?>
        <th class="<?= $Page->voucher->headerCellClass() ?>"><div role="button" data-sort="<?= HtmlEncode($Page->voucher->Name) ?>" data-sort-type="1" data-sort-order="<?= $Page->voucher->getNextSort() ?>">
            <div class="ew-table-header-btn">
                <span class="ew-table-header-caption"><?= $Page->voucher->caption() ?></span>
                <span class="ew-table-header-sort"><?= $Page->voucher->getSortIcon() ?></span>
            </div>
        </th>
    <?php } ?>
<?php } ?>
