...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fgajismkedit = new ew.Form("fgajismkedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fgajismkedit;

    // Add fields
    var fields = currentTable.fields;
    fgajismkedit.addFields([
    ...
    // Form_CustomValidate
    fgajismkedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fgajismkedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fgajismkedit.lists.bulan = <?= $Page->bulan->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fgajismkedit" id="fgajismkedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_gajismk_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_gajismk_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="gajismk" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_gajismk_bulan">
    <select
        ...
        data-select2-id="fgajismkedit_x_bulan"
        data-table="gajismk"
        data-field="x_bulan"
...
<script>
loadjs.ready("fgajismkedit", function() {
    var options = { name: "x_bulan", selectId: "fgajismkedit_x_bulan" };
    if (fgajismkedit.lists.bulan.lookupOptions.length) {
        options.data = { id: "x_bulan", form: "fgajismkedit" };
    } else {
        options.ajax = { id: "x_bulan", form: "fgajismkedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("gajismk_detil", explode(",", $Page->getCurrentDetailTable())) && $gajismk_detil->DetailEdit) {
?>
