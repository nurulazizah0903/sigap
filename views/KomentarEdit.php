...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fkomentaredit = new ew.Form("fkomentaredit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fkomentaredit;

    // Add fields
    var fields = currentTable.fields;
    fkomentaredit.addFields([
    ...
    // Form_CustomValidate
    fkomentaredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fkomentaredit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fkomentaredit.lists.pegawai = <?= $Page->pegawai->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fkomentaredit" id="fkomentaredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_komentar_pegawai">
    <select
        ...
        data-select2-id="fkomentaredit_x_pegawai"
        data-table="komentar"
...
<script>
loadjs.ready("fkomentaredit", function() {
    var options = { name: "x_pegawai", selectId: "fkomentaredit_x_pegawai" };
    if (fkomentaredit.lists.pegawai.lookupOptions.length) {
        options.data = { id: "x_pegawai", form: "fkomentaredit" };
    } else {
        options.ajax = { id: "x_pegawai", form: "fkomentaredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="komentar" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
