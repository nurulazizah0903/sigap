...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpotonganedit = new ew.Form("fpotonganedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fpotonganedit;

    // Add fields
    var fields = currentTable.fields;
    fpotonganedit.addFields([
    ...
    // Form_CustomValidate
    fpotonganedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpotonganedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpotonganedit.lists.nama = <?= $Page->nama->toClientList($Page) ?>;
    fpotonganedit.lists.jenjang_id = <?= $Page->jenjang_id->toClientList($Page) ?>;
    fpotonganedit.lists.jabatan_id = <?= $Page->jabatan_id->toClientList($Page) ?>;
    fpotonganedit.lists.u_by = <?= $Page->u_by->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpotonganedit" id="fpotonganedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php $Page->nama->EditAttrs->prepend("onchange", "ew.autoFill(this);"); ?>
    <select
        ...
        data-select2-id="fpotonganedit_x_nama"
        data-table="potongan"
        data-field="x_nama"
...
<script>
loadjs.ready("fpotonganedit", function() {
    var options = { name: "x_nama", selectId: "fpotonganedit_x_nama" };
    if (fpotonganedit.lists.nama.lookupOptions.length) {
        options.data = { id: "x_nama", form: "fpotonganedit" };
    } else {
        options.ajax = { id: "x_nama", form: "fpotonganedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_potongan_jenjang_id">
    <select
        ...
        data-select2-id="fpotonganedit_x_jenjang_id"
        data-table="potongan"
...
<script>
loadjs.ready("fpotonganedit", function() {
    var options = { name: "x_jenjang_id", selectId: "fpotonganedit_x_jenjang_id" };
    if (fpotonganedit.lists.jenjang_id.lookupOptions.length) {
        options.data = { id: "x_jenjang_id", form: "fpotonganedit" };
    } else {
        options.ajax = { id: "x_jenjang_id", form: "fpotonganedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_potongan_jabatan_id">
    <select
        ...
        data-select2-id="fpotonganedit_x_jabatan_id"
        data-table="potongan"
...
<script>
loadjs.ready("fpotonganedit", function() {
    var options = { name: "x_jabatan_id", selectId: "fpotonganedit_x_jabatan_id" };
    if (fpotonganedit.lists.jabatan_id.lookupOptions.length) {
        options.data = { id: "x_jabatan_id", form: "fpotonganedit" };
    } else {
        options.ajax = { id: "x_jabatan_id", form: "fpotonganedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="potongan" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
