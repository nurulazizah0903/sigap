...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    flemburadd = new ew.Form("flemburadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = flemburadd;

    // Add fields
    var fields = currentTable.fields;
    flemburadd.addFields([
    ...
    // Form_CustomValidate
    flemburadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    flemburadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    flemburadd.lists.pegawai = <?= $Page->pegawai->toClientList($Page) ?>;
    flemburadd.lists.proyek = <?= $Page->proyek->toClientList($Page) ?>;
    flemburadd.lists.pm = <?= $Page->pm->toClientList($Page) ?>;
    flemburadd.lists.jenis = <?= $Page->jenis->toClientList($Page) ?>;
    flemburadd.lists.disetujui = <?= $Page->disetujui->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="flemburadd" id="flemburadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready("flemburadd", function() {
    flemburadd.createAutoSuggest(Object.assign({"id":"x_pegawai","forceSelect":false}, ew.vars.tables.lembur.fields.pegawai.autoSuggestOptions));
});
</script>
...
<span id="el_lembur_proyek">
    <select
        ...
        data-select2-id="flemburadd_x_proyek"
        data-table="lembur"
        data-field="x_proyek"
...
<script>
loadjs.ready("flemburadd", function() {
    var options = { name: "x_proyek", selectId: "flemburadd_x_proyek" };
    if (flemburadd.lists.proyek.lookupOptions.length) {
        options.data = { id: "x_proyek", form: "flemburadd" };
    } else {
        options.ajax = { id: "x_proyek", form: "flemburadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_lembur_pm">
    <select
        ...
        data-select2-id="flemburadd_x_pm"
        data-table="lembur"
        data-field="x_pm"
...
<script>
loadjs.ready("flemburadd", function() {
    var options = { name: "x_pm", selectId: "flemburadd_x_pm" };
    if (flemburadd.lists.pm.lookupOptions.length) {
        options.data = { id: "x_pm", form: "flemburadd" };
    } else {
        options.ajax = { id: "x_pm", form: "flemburadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["flemburadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("flemburadd", "x_tgl_awal_lembur", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["flemburadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("flemburadd", "x_tgl_akhir_lembur", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_lembur_jenis">
    <select
        ...
        data-select2-id="flemburadd_x_jenis"
        data-table="lembur"
        data-field="x_jenis"
...
<script>
loadjs.ready("flemburadd", function() {
    var options = { name: "x_jenis", selectId: "flemburadd_x_jenis" };
    if (flemburadd.lists.jenis.lookupOptions.length) {
        options.data = { id: "x_jenis", form: "flemburadd" };
    } else {
        options.ajax = { id: "x_jenis", form: "flemburadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
