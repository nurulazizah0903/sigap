...
// Page object
$GajismpDetilPreview = &$Page;
?>
<script>
ew.deepAssign(ew.vars, { tables: { gajismp_detil: <?= JsonEncode($Page->toClientVar()) ?> } });
</script>
...
<?php if ($Page->pid->Visible) { // pid ?>
    ...
    <?php } else { ?>
        <th class="<?= $Page->pid->headerCellClass() ?>"><div role="button" data-sort="<?= HtmlEncode($Page->pid->Name) ?>" data-sort-type="1" data-sort-order="<?= $Page->pid->getNextSort() ?>">
            <div class="ew-table-header-btn">
                <span class="ew-table-header-caption"><?= $Page->pid->caption() ?></span>
                <span class="ew-table-header-sort"><?= $Page->pid->getSortIcon() ?></span>
            </div>
        </th>
    <?php } ?>
<?php } ?>
