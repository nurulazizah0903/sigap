...
// Page object
$JenisDinasluarEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { jenis_dinasluar: currentTable } });
var currentForm, currentPageID;
var fjenis_dinasluaredit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fjenis_dinasluaredit = new ew.Form("fjenis_dinasluaredit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fjenis_dinasluaredit;

    // Add fields
    var fields = currentTable.fields;
    fjenis_dinasluaredit.addFields([
    ...
    // Form_CustomValidate
    fjenis_dinasluaredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fjenis_dinasluaredit.validateRequired = ew.CLIENT_VALIDATE;
...
$Page->showMessage();
?>
<form name="fjenis_dinasluaredit" id="fjenis_dinasluaredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_jenis_dinasluar_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_jenis_dinasluar_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="jenis_dinasluar" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
