...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fgajisdadd = new ew.Form("fgajisdadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fgajisdadd;

    // Add fields
    var fields = currentTable.fields;
    fgajisdadd.addFields([
    ...
    // Form_CustomValidate
    fgajisdadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fgajisdadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fgajisdadd.lists.bulan = <?= $Page->bulan->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fgajisdadd" id="fgajisdadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_gajisd_bulan">
    <select
        ...
        data-select2-id="fgajisdadd_x_bulan"
        data-table="gajisd"
        data-field="x_bulan"
...
<script>
loadjs.ready("fgajisdadd", function() {
    var options = { name: "x_bulan", selectId: "fgajisdadd_x_bulan" };
    if (fgajisdadd.lists.bulan.lookupOptions.length) {
        options.data = { id: "x_bulan", form: "fgajisdadd" };
    } else {
        options.ajax = { id: "x_bulan", form: "fgajisdadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("gajisd_detil", explode(",", $Page->getCurrentDetailTable())) && $gajisd_detil->DetailAdd) {
?>
