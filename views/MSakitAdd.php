...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fm_sakitadd = new ew.Form("fm_sakitadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fm_sakitadd;

    // Add fields
    var fields = currentTable.fields;
    fm_sakitadd.addFields([
    ...
    // Form_CustomValidate
    fm_sakitadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fm_sakitadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fm_sakitadd.lists.jenjang_id = <?= $Page->jenjang_id->toClientList($Page) ?>;
    fm_sakitadd.lists.jabatan = <?= $Page->jabatan->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fm_sakitadd" id="fm_sakitadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_m_sakit_jenjang_id">
    <select
        ...
        data-select2-id="fm_sakitadd_x_jenjang_id"
        data-table="m_sakit"
...
loadjs.ready("fm_sakitadd", function() {
    var options = { name: "x_jenjang_id", selectId: "fm_sakitadd_x_jenjang_id" };
    if (fm_sakitadd.lists.jenjang_id.lookupOptions.length) {
        options.data = { id: "x_jenjang_id", form: "fm_sakitadd" };
    } else {
        options.ajax = { id: "x_jenjang_id", form: "fm_sakitadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_m_sakit_jabatan">
    <select
        ...
        data-select2-id="fm_sakitadd_x_jabatan"
        data-table="m_sakit"
...
<script>
loadjs.ready("fm_sakitadd", function() {
    var options = { name: "x_jabatan", selectId: "fm_sakitadd_x_jabatan" };
    if (fm_sakitadd.lists.jabatan.lookupOptions.length) {
        options.data = { id: "x_jabatan", form: "fm_sakitadd" };
    } else {
        options.ajax = { id: "x_jabatan", form: "fm_sakitadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
