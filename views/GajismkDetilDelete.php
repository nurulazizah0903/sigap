...
// Page object
$GajismkDetilDelete = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { gajismk_detil: currentTable } });
var currentForm, currentPageID;
var fgajismk_detildelete;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fgajismk_detildelete = new ew.Form("fgajismk_detildelete", "delete");
    currentPageID = ew.PAGE_ID = "delete";
    currentForm = fgajismk_detildelete;
...
$Page->showMessage();
?>
<form name="fgajismk_detildelete" id="fgajismk_detildelete" class="ew-form ew-delete-form" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->pegawai_id->Visible) { // pegawai_id ?>
        <th class="<?= $Page->pegawai_id->headerCellClass() ?>"><span id="elh_gajismk_detil_pegawai_id" class="gajismk_detil_pegawai_id"><?= $Page->pegawai_id->caption() ?></span></th>
<?php } ?>
<?php if ($Page->jabatan_id->Visible) { // jabatan_id ?>
        <th class="<?= $Page->jabatan_id->headerCellClass() ?>"><span id="elh_gajismk_detil_jabatan_id" class="gajismk_detil_jabatan_id"><?= $Page->jabatan_id->caption() ?></span></th>
<?php } ?>
<?php if ($Page->masakerja->Visible) { // masakerja ?>
        <th class="<?= $Page->masakerja->headerCellClass() ?>"><span id="elh_gajismk_detil_masakerja" class="gajismk_detil_masakerja"><?= $Page->masakerja->caption() ?></span></th>
<?php } ?>
<?php if ($Page->jumngajar->Visible) { // jumngajar ?>
        <th class="<?= $Page->jumngajar->headerCellClass() ?>"><span id="elh_gajismk_detil_jumngajar" class="gajismk_detil_jumngajar"><?= $Page->jumngajar->caption() ?></span></th>
<?php } ?>
<?php if ($Page->ijin->Visible) { // ijin ?>
        <th class="<?= $Page->ijin->headerCellClass() ?>"><span id="elh_gajismk_detil_ijin" class="gajismk_detil_ijin"><?= $Page->ijin->caption() ?></span></th>
<?php } ?>
<?php if ($Page->tunjangan_wkosis->Visible) { // tunjangan_wkosis ?>
        <th class="<?= $Page->tunjangan_wkosis->headerCellClass() ?>"><span id="elh_gajismk_detil_tunjangan_wkosis" class="gajismk_detil_tunjangan_wkosis"><?= $Page->tunjangan_wkosis->caption() ?></span></th>
<?php } ?>
<?php if ($Page->nominal_baku->Visible) { // nominal_baku ?>
        <th class="<?= $Page->nominal_baku->headerCellClass() ?>"><span id="elh_gajismk_detil_nominal_baku" class="gajismk_detil_nominal_baku"><?= $Page->nominal_baku->caption() ?></span></th>
<?php } ?>
<?php if ($Page->baku->Visible) { // baku ?>
        <th class="<?= $Page->baku->headerCellClass() ?>"><span id="elh_gajismk_detil_baku" class="gajismk_detil_baku"><?= $Page->baku->caption() ?></span></th>
<?php } ?>
<?php if ($Page->kehadiran->Visible) { // kehadiran ?>
        <th class="<?= $Page->kehadiran->headerCellClass() ?>"><span id="elh_gajismk_detil_kehadiran" class="gajismk_detil_kehadiran"><?= $Page->kehadiran->caption() ?></span></th>
<?php } ?>
<?php if ($Page->prestasi->Visible) { // prestasi ?>
        <th class="<?= $Page->prestasi->headerCellClass() ?>"><span id="elh_gajismk_detil_prestasi" class="gajismk_detil_prestasi"><?= $Page->prestasi->caption() ?></span></th>
<?php } ?>
<?php if ($Page->jumlahgaji->Visible) { // jumlahgaji ?>
        <th class="<?= $Page->jumlahgaji->headerCellClass() ?>"><span id="elh_gajismk_detil_jumlahgaji" class="gajismk_detil_jumlahgaji"><?= $Page->jumlahgaji->caption() ?></span></th>
<?php } ?>
<?php if ($Page->jumgajitotal->Visible) { // jumgajitotal ?>
        <th class="<?= $Page->jumgajitotal->headerCellClass() ?>"><span id="elh_gajismk_detil_jumgajitotal" class="gajismk_detil_jumgajitotal"><?= $Page->jumgajitotal->caption() ?></span></th>
<?php } ?>
<?php if ($Page->potongan1->Visible) { // potongan1 ?>
        <th class="<?= $Page->potongan1->headerCellClass() ?>"><span id="elh_gajismk_detil_potongan1" class="gajismk_detil_potongan1"><?= $Page->potongan1->caption() ?></span></th>
<?php } ?>
<?php if ($Page->potongan2->Visible) { // potongan2 ?>
        <th class="<?= $Page->potongan2->headerCellClass() ?>"><span id="elh_gajismk_detil_potongan2" class="gajismk_detil_potongan2"><?= $Page->potongan2->caption() ?></span></th>
<?php } ?>
<?php if ($Page->jumlahterima->Visible) { // jumlahterima ?>
        <th class="<?= $Page->jumlahterima->headerCellClass() ?>"><span id="elh_gajismk_detil_jumlahterima" class="gajismk_detil_jumlahterima"><?= $Page->jumlahterima->caption() ?></span></th>
<?php } ?>
    </tr>
    </thead>
    <tbody>
