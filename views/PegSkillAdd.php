...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpeg_skilladd = new ew.Form("fpeg_skilladd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fpeg_skilladd;

    // Add fields
    var fields = currentTable.fields;
    fpeg_skilladd.addFields([
    ...
    // Form_CustomValidate
    fpeg_skilladd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpeg_skilladd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpeg_skilladd.lists.c_by = <?= $Page->c_by->toClientList($Page) ?>;
    fpeg_skilladd.lists.u_by = <?= $Page->u_by->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpeg_skilladd" id="fpeg_skilladd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_peg_skill_c_by">
    <select
        ...
        data-select2-id="fpeg_skilladd_x_c_by"
...
<script>
loadjs.ready("fpeg_skilladd", function() {
    var options = { name: "x_c_by", selectId: "fpeg_skilladd_x_c_by" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpeg_skilladd.lists.c_by.lookupOptions.length) {
        options.data = { id: "x_c_by", form: "fpeg_skilladd" };
    } else {
        options.ajax = { id: "x_c_by", form: "fpeg_skilladd", limit: ew.LOOKUP_PAGE_SIZE };
    }
