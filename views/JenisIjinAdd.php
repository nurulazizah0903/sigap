...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fjenis_ijinadd = new ew.Form("fjenis_ijinadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fjenis_ijinadd;

    // Add fields
    var fields = currentTable.fields;
    fjenis_ijinadd.addFields([
    ...
    // Form_CustomValidate
    fjenis_ijinadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fjenis_ijinadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fjenis_ijinadd.lists.jenjang_id = <?= $Page->jenjang_id->toClientList($Page) ?>;
    fjenis_ijinadd.lists.jabatan_id = <?= $Page->jabatan_id->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fjenis_ijinadd" id="fjenis_ijinadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_jenis_ijin_jenjang_id">
    <select
        ...
        data-select2-id="fjenis_ijinadd_x_jenjang_id"
...
loadjs.ready("fjenis_ijinadd", function() {
    var options = { name: "x_jenjang_id", selectId: "fjenis_ijinadd_x_jenjang_id" };
    if (fjenis_ijinadd.lists.jenjang_id.lookupOptions.length) {
        options.data = { id: "x_jenjang_id", form: "fjenis_ijinadd" };
    } else {
        options.ajax = { id: "x_jenjang_id", form: "fjenis_ijinadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_jenis_ijin_jabatan_id">
    <select
        ...
        data-select2-id="fjenis_ijinadd_x_jabatan_id"
        data-table="jenis_ijin"
...
<script>
loadjs.ready("fjenis_ijinadd", function() {
    var options = { name: "x_jabatan_id", selectId: "fjenis_ijinadd_x_jabatan_id" };
    if (fjenis_ijinadd.lists.jabatan_id.lookupOptions.length) {
        options.data = { id: "x_jabatan_id", form: "fjenis_ijinadd" };
    } else {
        options.ajax = { id: "x_jabatan_id", form: "fjenis_ijinadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
