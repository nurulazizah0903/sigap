...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    freimburshedit = new ew.Form("freimburshedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = freimburshedit;

    // Add fields
    var fields = currentTable.fields;
    freimburshedit.addFields([
    ...
    // Form_CustomValidate
    freimburshedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    freimburshedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    freimburshedit.lists.pegawai = <?= $Page->pegawai->toClientList($Page) ?>;
    freimburshedit.lists.disetujui = <?= $Page->disetujui->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="freimburshedit" id="freimburshedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_reimbursh_pegawai">
    <select
        ...
        data-select2-id="freimburshedit_x_pegawai"
...
loadjs.ready("freimburshedit", function() {
    var options = { name: "x_pegawai", selectId: "freimburshedit_x_pegawai" };
    if (freimburshedit.lists.pegawai.lookupOptions.length) {
        options.data = { id: "x_pegawai", form: "freimburshedit" };
    } else {
        options.ajax = { id: "x_pegawai", form: "freimburshedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["freimburshedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("freimburshedit", "x_tgl_pengajuan", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["freimburshedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("freimburshedit", "x_tgl_pembayaran", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="reimbursh" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
