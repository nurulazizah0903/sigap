...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fdinasluaredit = new ew.Form("fdinasluaredit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fdinasluaredit;

    // Add fields
    var fields = currentTable.fields;
    fdinasluaredit.addFields([
    ...
    // Form_CustomValidate
    fdinasluaredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fdinasluaredit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fdinasluaredit.lists.pegawai = <?= $Page->pegawai->toClientList($Page) ?>;
    fdinasluaredit.lists.pm = <?= $Page->pm->toClientList($Page) ?>;
    fdinasluaredit.lists.proyek = <?= $Page->proyek->toClientList($Page) ?>;
    fdinasluaredit.lists.jenis = <?= $Page->jenis->toClientList($Page) ?>;
    fdinasluaredit.lists.disetujui = <?= $Page->disetujui->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fdinasluaredit" id="fdinasluaredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_dinasluar_pegawai">
    <select
        ...
        data-select2-id="fdinasluaredit_x_pegawai"
...
<script>
loadjs.ready("fdinasluaredit", function() {
    var options = { name: "x_pegawai", selectId: "fdinasluaredit_x_pegawai" };
    if (fdinasluaredit.lists.pegawai.lookupOptions.length) {
        options.data = { id: "x_pegawai", form: "fdinasluaredit" };
    } else {
        options.ajax = { id: "x_pegawai", form: "fdinasluaredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_dinasluar_pm">
    <select
        ...
        data-select2-id="fdinasluaredit_x_pm"
        data-table="dinasluar"
        data-field="x_pm"
...
<script>
loadjs.ready("fdinasluaredit", function() {
    var options = { name: "x_pm", selectId: "fdinasluaredit_x_pm" };
    if (fdinasluaredit.lists.pm.lookupOptions.length) {
        options.data = { id: "x_pm", form: "fdinasluaredit" };
    } else {
        options.ajax = { id: "x_pm", form: "fdinasluaredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_dinasluar_proyek">
    <select
        ...
        data-select2-id="fdinasluaredit_x_proyek"
        data-table="dinasluar"
        data-field="x_proyek"
...
<script>
loadjs.ready("fdinasluaredit", function() {
    var options = { name: "x_proyek", selectId: "fdinasluaredit_x_proyek" };
    if (fdinasluaredit.lists.proyek.lookupOptions.length) {
        options.data = { id: "x_proyek", form: "fdinasluaredit" };
    } else {
        options.ajax = { id: "x_proyek", form: "fdinasluaredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fdinasluaredit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdinasluaredit", "x_tgl_dl_awal", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fdinasluaredit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdinasluaredit", "x_tgl_dl_akhir", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_dinasluar_jenis">
    <select
        ...
        data-select2-id="fdinasluaredit_x_jenis"
        data-table="dinasluar"
        data-field="x_jenis"
...
<script>
loadjs.ready("fdinasluaredit", function() {
    var options = { name: "x_jenis", selectId: "fdinasluaredit_x_jenis" };
    if (fdinasluaredit.lists.jenis.lookupOptions.length) {
        options.data = { id: "x_jenis", form: "fdinasluaredit" };
    } else {
        options.ajax = { id: "x_jenis", form: "fdinasluaredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="dinasluar" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
