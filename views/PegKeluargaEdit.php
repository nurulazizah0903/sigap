...
// Page object
$PegKeluargaEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { peg_keluarga: currentTable } });
var currentForm, currentPageID;
var fpeg_keluargaedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpeg_keluargaedit = new ew.Form("fpeg_keluargaedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fpeg_keluargaedit;

    // Add fields
    var fields = currentTable.fields;
    fpeg_keluargaedit.addFields([
    ...
    // Form_CustomValidate
    fpeg_keluargaedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpeg_keluargaedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpeg_keluargaedit.lists.jen_kel = <?= $Page->jen_kel->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpeg_keluargaedit" id="fpeg_keluargaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready(["fpeg_keluargaedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpeg_keluargaedit", "x_tgl_lahir", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="peg_keluarga" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
