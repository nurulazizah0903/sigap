...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fijinedit = new ew.Form("fijinedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fijinedit;
    ...
    // Form_CustomValidate
    fijinedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fijinedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fijinedit.lists.pegawai = <?= $Page->pegawai->toClientList($Page) ?>;
    fijinedit.lists.jenis = <?= $Page->jenis->toClientList($Page) ?>;
    fijinedit.lists.disetujui = <?= $Page->disetujui->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fijinedit" id="fijinedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_ijin_pegawai">
    <select
        ...
        data-select2-id="fijinedit_x_pegawai"
        data-table="ijin"
...
loadjs.ready("fijinedit", function() {
    var options = { name: "x_pegawai", selectId: "fijinedit_x_pegawai" };
    if (fijinedit.lists.pegawai.lookupOptions.length) {
        options.data = { id: "x_pegawai", form: "fijinedit" };
    } else {
        options.ajax = { id: "x_pegawai", form: "fijinedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fijinedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fijinedit", "x_tgl_ijin_awal", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fijinedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fijinedit", "x_tgl_ijin_akhir", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_ijin_jenis">
    <select
        ...
        data-select2-id="fijinedit_x_jenis"
        data-table="ijin"
        data-field="x_jenis"
...
<script>
loadjs.ready("fijinedit", function() {
    var options = { name: "x_jenis", selectId: "fijinedit_x_jenis" };
    if (fijinedit.lists.jenis.lookupOptions.length) {
        options.data = { id: "x_jenis", form: "fijinedit" };
    } else {
        options.ajax = { id: "x_jenis", form: "fijinedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="ijin" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
