...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fgaji_pokokadd = new ew.Form("fgaji_pokokadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fgaji_pokokadd;

    // Add fields
    var fields = currentTable.fields;
    fgaji_pokokadd.addFields([
    ...
    // Form_CustomValidate
    fgaji_pokokadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fgaji_pokokadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fgaji_pokokadd.lists.jenjang_id = <?= $Page->jenjang_id->toClientList($Page) ?>;
    fgaji_pokokadd.lists.ijazah_id = <?= $Page->ijazah_id->toClientList($Page) ?>;
    fgaji_pokokadd.lists.jenis = <?= $Page->jenis->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fgaji_pokokadd" id="fgaji_pokokadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_gaji_pokok_jenjang_id">
    <select
        ...
        data-select2-id="fgaji_pokokadd_x_jenjang_id"
...
<script>
loadjs.ready("fgaji_pokokadd", function() {
    var options = { name: "x_jenjang_id", selectId: "fgaji_pokokadd_x_jenjang_id" };
    if (fgaji_pokokadd.lists.jenjang_id.lookupOptions.length) {
        options.data = { id: "x_jenjang_id", form: "fgaji_pokokadd" };
    } else {
        options.ajax = { id: "x_jenjang_id", form: "fgaji_pokokadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_gaji_pokok_ijazah_id">
    <select
        ...
        data-select2-id="fgaji_pokokadd_x_ijazah_id"
        data-table="gaji_pokok"
...
<script>
loadjs.ready("fgaji_pokokadd", function() {
    var options = { name: "x_ijazah_id", selectId: "fgaji_pokokadd_x_ijazah_id" };
    if (fgaji_pokokadd.lists.ijazah_id.lookupOptions.length) {
        options.data = { id: "x_ijazah_id", form: "fgaji_pokokadd" };
    } else {
        options.ajax = { id: "x_ijazah_id", form: "fgaji_pokokadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_gaji_pokok_jenis">
    <select
        ...
        data-select2-id="fgaji_pokokadd_x_jenis"
        data-table="gaji_pokok"
        data-field="x_jenis"
...
<script>
loadjs.ready("fgaji_pokokadd", function() {
    var options = { name: "x_jenis", selectId: "fgaji_pokokadd_x_jenis" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fgaji_pokokadd.lists.jenis.lookupOptions.length) {
        options.data = { id: "x_jenis", form: "fgaji_pokokadd" };
    } else {
        options.ajax = { id: "x_jenis", form: "fgaji_pokokadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
