...
var fpotongan_sdedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpotongan_sdedit = new ew.Form("fpotongan_sdedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fpotongan_sdedit;

    // Add fields
    var fields = currentTable.fields;
    fpotongan_sdedit.addFields([
    ...
    // Form_CustomValidate
    fpotongan_sdedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpotongan_sdedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpotongan_sdedit.lists.u_by = <?= $Page->u_by->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpotongan_sdedit" id="fpotongan_sdedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_potongan_sd_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_potongan_sd_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="potongan_sd" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
