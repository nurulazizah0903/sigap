...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fberitaadd = new ew.Form("fberitaadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fberitaadd;
    ...
    fberitaadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fberitaadd.lists.grup = <?= $Page->grup->toClientList($Page) ?>;
    fberitaadd.lists.c_by = <?= $Page->c_by->toClientList($Page) ?>;
...
<span id="el_berita_grup">
    <select
        ...
        data-select2-id="fberitaadd_x_grup"
        data-table="berita"
        data-field="x_grup"
...
<script>
loadjs.ready("fberitaadd", function() {
    var options = { name: "x_grup", selectId: "fberitaadd_x_grup" };
    if (fberitaadd.lists.grup.lookupOptions.length) {
        options.data = { id: "x_grup", form: "fberitaadd" };
    } else {
        options.ajax = { id: "x_grup", form: "fberitaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("komentar", explode(",", $Page->getCurrentDetailTable())) && $komentar->DetailAdd) {
?>
