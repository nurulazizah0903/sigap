...
// Page object
$DaftarbarangAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { daftarbarang: currentTable } });
var currentForm, currentPageID;
var fdaftarbarangadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fdaftarbarangadd = new ew.Form("fdaftarbarangadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fdaftarbarangadd;

    // Add fields
    var fields = currentTable.fields;
    fdaftarbarangadd.addFields([
    ...
    // Form_CustomValidate
    fdaftarbarangadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fdaftarbarangadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fdaftarbarangadd.lists.jenis = <?= $Page->jenis->toClientList($Page) ?>;
    fdaftarbarangadd.lists.pemegang = <?= $Page->pemegang->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fdaftarbarangadd" id="fdaftarbarangadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_daftarbarang_jenis">
    <select
        ...
        data-select2-id="fdaftarbarangadd_x_jenis"
...
loadjs.ready("fdaftarbarangadd", function() {
    var options = { name: "x_jenis", selectId: "fdaftarbarangadd_x_jenis" };
    if (fdaftarbarangadd.lists.jenis.lookupOptions.length) {
        options.data = { id: "x_jenis", form: "fdaftarbarangadd" };
    } else {
        options.ajax = { id: "x_jenis", form: "fdaftarbarangadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fdaftarbarangadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdaftarbarangadd", "x_tgl_terima", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fdaftarbarangadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdaftarbarangadd", "x_tgl_beli", ew.deepAssign({"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_daftarbarang_pemegang">
    <select
        ...
        data-select2-id="fdaftarbarangadd_x_pemegang"
        data-table="daftarbarang"
...
<script>
loadjs.ready("fdaftarbarangadd", function() {
    var options = { name: "x_pemegang", selectId: "fdaftarbarangadd_x_pemegang" };
    if (fdaftarbarangadd.lists.pemegang.lookupOptions.length) {
        options.data = { id: "x_pemegang", form: "fdaftarbarangadd" };
    } else {
        options.ajax = { id: "x_pemegang", form: "fdaftarbarangadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
