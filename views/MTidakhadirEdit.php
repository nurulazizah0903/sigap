...
// Page object
$MTidakhadirEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { m_tidakhadir: currentTable } });
var currentForm, currentPageID;
var fm_tidakhadiredit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fm_tidakhadiredit = new ew.Form("fm_tidakhadiredit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fm_tidakhadiredit;

    // Add fields
    var fields = currentTable.fields;
    fm_tidakhadiredit.addFields([
    ...
    // Form_CustomValidate
    fm_tidakhadiredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fm_tidakhadiredit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fm_tidakhadiredit.lists.jenjang_id = <?= $Page->jenjang_id->toClientList($Page) ?>;
    fm_tidakhadiredit.lists.jabatan_id = <?= $Page->jabatan_id->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fm_tidakhadiredit" id="fm_tidakhadiredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_m_tidakhadir_jenjang_id">
    <select
        ...
        data-select2-id="fm_tidakhadiredit_x_jenjang_id"
...
<script>
loadjs.ready("fm_tidakhadiredit", function() {
    var options = { name: "x_jenjang_id", selectId: "fm_tidakhadiredit_x_jenjang_id" };
    if (fm_tidakhadiredit.lists.jenjang_id.lookupOptions.length) {
        options.data = { id: "x_jenjang_id", form: "fm_tidakhadiredit" };
    } else {
        options.ajax = { id: "x_jenjang_id", form: "fm_tidakhadiredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_m_tidakhadir_jabatan_id">
    <select
        ...
        data-select2-id="fm_tidakhadiredit_x_jabatan_id"
        data-table="m_tidakhadir"
...
<script>
loadjs.ready("fm_tidakhadiredit", function() {
    var options = { name: "x_jabatan_id", selectId: "fm_tidakhadiredit_x_jabatan_id" };
    if (fm_tidakhadiredit.lists.jabatan_id.lookupOptions.length) {
        options.data = { id: "x_jabatan_id", form: "fm_tidakhadiredit" };
    } else {
        options.ajax = { id: "x_jabatan_id", form: "fm_tidakhadiredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="m_tidakhadir" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
