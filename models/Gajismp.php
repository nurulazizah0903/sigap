...
class Gajismp extends DbTable
{
    ...
    public function __construct()
    {
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->bulan->Lookup = new Lookup('bulan', 'bulan', false, 'id', ["bulan","","",""], [], [], [], [], [], [], '', '', "`bulan`");
                break;
            default:
                $this->bulan->Lookup = new Lookup('bulan', 'bulan', false, 'id', ["bulan","","",""], [], [], [], [], [], [], '', '', "`bulan`");
                break;
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "gajismp_detil") {
            $detailUrl = Container("gajismp_detil")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "GajismpList";
        }
        return $detailUrl;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("gajismp_detil")->loadRs("`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("gajismp_detil")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("gajismp_detil")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("gajismp_detil")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("GajismpList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "GajismpEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "GajismpAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("GajismpView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("GajismpView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "GajismpAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "GajismpAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("GajismpEdit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("GajismpEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("GajismpAdd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("GajismpAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("GajismpDelete", $this->getUrlParm());
    }
    ...
    public function renderEditRow()
    {
        ...
        if (strval($this->tahun->EditValue) != "" && is_numeric($this->tahun->EditValue)) {
            $this->tahun->EditValue = FormatNumber($this->tahun->EditValue, null);
        }
