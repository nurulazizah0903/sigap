 ...
 * Page class
 */
class ReimburshList extends Reimbursh
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ReimburshList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ReimburshAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ReimburshDelete";
        $this->MultiUpdateUrl = "ReimburshUpdate";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "freimburshsrch", $filters);
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->rek_tujuan;
        $searchFlds[] = &$this->disetujui;
        $searchFlds[] = &$this->pembayar;
        $searchFlds[] = &$this->terbayar;
        $searchFlds[] = &$this->bukti1;
        $searchFlds[] = &$this->bukti2;
        $searchFlds[] = &$this->bukti3;
        $searchFlds[] = &$this->bukti4;
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"freimburshlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"freimburshlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("tgl", $this->createColumnOption("tgl"));
            $option->add("total_pengajuan", $this->createColumnOption("total_pengajuan"));
            $option->add("tgl_pengajuan", $this->createColumnOption("tgl_pengajuan"));
            $option->add("jenis", $this->createColumnOption("jenis"));
            $option->add("rek_tujuan", $this->createColumnOption("rek_tujuan"));
            $option->add("disetujui", $this->createColumnOption("disetujui"));
            $option->add("pembayar", $this->createColumnOption("pembayar"));
            $option->add("terbayar", $this->createColumnOption("terbayar"));
            $option->add("tgl_pembayaran", $this->createColumnOption("tgl_pembayaran"));
            $option->add("jumlah_dibayar", $this->createColumnOption("jumlah_dibayar"));
            $option->add("bukti1", $this->createColumnOption("bukti1"));
            $option->add("bukti2", $this->createColumnOption("bukti2"));
            $option->add("bukti3", $this->createColumnOption("bukti3"));
            $option->add("bukti4", $this->createColumnOption("bukti4"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"freimburshsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"freimburshsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="freimburshlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"freimburshsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
