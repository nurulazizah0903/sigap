 ...
 * Page class
 */
class TotalgajiList extends Totalgaji
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'totalgaji';

    // Page object name
    public $PageObjName = "TotalgajiList";
    ...
    public $FormName = "ftotalgajilist";
    ...
    public function __construct()
    {
        ...
        // Table object (totalgaji)
        if (!isset($GLOBALS["totalgaji"]) || get_class($GLOBALS["totalgaji"]) == PROJECT_NAMESPACE . "totalgaji") {
            $GLOBALS["totalgaji"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "TotalgajiAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "TotalgajiDelete";
        $this->MultiUpdateUrl = "TotalgajiUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'totalgaji');
        }
    ...
    public function run()
    {
        ...
        $this->valuejabatan->setVisibility();
        $this->valuetunjangan->setVisibility();
        $this->value_lembur->setVisibility();
        $this->Column7->setVisibility();
        $this->Column8->setVisibility();
        $this->Column9->setVisibility();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->valuejabatan); // valuejabatan
            $this->updateSort($this->valuetunjangan); // valuetunjangan
            $this->updateSort($this->value_lembur); // value_lembur
            $this->updateSort($this->Column7); // Column 7
            $this->updateSort($this->Column8); // Column 8
            $this->updateSort($this->Column9); // Column 9
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->valuejabatan->setSort("");
                $this->valuetunjangan->setSort("");
                $this->value_lembur->setSort("");
                $this->Column7->setSort("");
                $this->Column8->setSort("");
                $this->Column9->setSort("");
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ftotalgajilist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ftotalgajilist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("valuejabatan", $this->createColumnOption("valuejabatan"));
            $option->add("valuetunjangan", $this->createColumnOption("valuetunjangan"));
            $option->add("value_lembur", $this->createColumnOption("value_lembur"));
            $option->add("Column 7", $this->createColumnOption("Column 7"));
            $option->add("Column 8", $this->createColumnOption("Column 8"));
            $option->add("Column 9", $this->createColumnOption("Column 9"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ftotalgajisrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ftotalgajisrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ftotalgajilist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        ...
        $row['valuejabatan'] = $this->valuejabatan->DefaultValue;
        $row['valuetunjangan'] = $this->valuetunjangan->DefaultValue;
        $row['value_lembur'] = $this->value_lembur->DefaultValue;
        $row['Column 7'] = $this->Column7->DefaultValue;
        $row['Column 8'] = $this->Column8->DefaultValue;
        $row['Column 9'] = $this->Column9->DefaultValue;
        return $row;
    }
