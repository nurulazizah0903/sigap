 ...
 * Page class
 */
class GajitkDetilList extends GajitkDetil
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "GajitkDetilList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "GajitkDetilAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "GajitkDetilDelete";
        $this->MultiUpdateUrl = "GajitkDetilUpdate";
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "gajitk") {
            $masterTbl = Container("gajitk");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("GajitkList"); // Return to master page
                return;
            } else {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fgajitk_detillist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fgajitk_detillist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("pegawai_id", $this->createColumnOption("pegawai_id"));
            $option->add("jabatan_id", $this->createColumnOption("jabatan_id"));
            $option->add("masakerja", $this->createColumnOption("masakerja"));
            $option->add("jumngajar", $this->createColumnOption("jumngajar"));
            $option->add("ijin", $this->createColumnOption("ijin"));
            $option->add("voucher", $this->createColumnOption("voucher"));
            $option->add("tunjangan_khusus", $this->createColumnOption("tunjangan_khusus"));
            $option->add("tunjangan_jabatan", $this->createColumnOption("tunjangan_jabatan"));
            $option->add("baku", $this->createColumnOption("baku"));
            $option->add("kehadiran", $this->createColumnOption("kehadiran"));
            $option->add("prestasi", $this->createColumnOption("prestasi"));
            $option->add("jumlahgaji", $this->createColumnOption("jumlahgaji"));
            $option->add("jumgajitotal", $this->createColumnOption("jumgajitotal"));
            $option->add("potongan1", $this->createColumnOption("potongan1"));
            $option->add("potongan2", $this->createColumnOption("potongan2"));
            $option->add("jumlahterima", $this->createColumnOption("jumlahterima"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fgajitk_detilsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fgajitk_detilsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fgajitk_detillist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        ...
        $row['voucher'] = $this->voucher->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->voucher->ViewValue = FormatNumber($this->voucher->ViewValue, $this->voucher->formatPattern());
