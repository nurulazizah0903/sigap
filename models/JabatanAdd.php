...
class JabatanAdd extends Jabatan
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("gajitunjangan", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // c_date
        $this->c_date->setDbValueDef($rsnew, UnFormatDateTime($this->c_date->CurrentValue, $this->c_date->formatPattern()), null, false);
        ...
        if ($addRow) {
            ...
            $detailPage = Container("GajitunjanganGrid");
            if (in_array("gajitunjangan", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->pidjabatan->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "gajitunjangan"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->pidjabatan->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
