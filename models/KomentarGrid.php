 ...
 * Page class
 */
class KomentarGrid extends Komentar
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "KomentarGrid";
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_komentar") &&
            $CurrentForm->hasValue("o_komentar") &&
            $this->komentar->CurrentValue != $this->komentar->DefaultValue &&
            !($this->komentar->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->komentar->CurrentValue == $this->komentar->getSessionValue())
        ) {
            return false;
        }
        if (!EmptyValue($this->gambar->Upload->Value)) {
            return false;
        }
        if (!EmptyValue($this->video->Upload->Value)) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->komentar->clearErrorMessage();
        $this->gambar->clearErrorMessage();
        $this->video->clearErrorMessage();
    }
    ...
    {
        $this->gambar->Upload->Index = $this->RowIndex;
        $this->video->Upload->Index = $this->RowIndex;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // komentar
            $this->komentar->setupEditAttributes();
            ...
            if (!EmptyValue($this->gambar->CurrentValue)) {
                if ($this->RowIndex == '$rowindex$') {
                    $this->gambar->Upload->FileName = "";
                } else {
                    $this->gambar->Upload->FileName = $this->gambar->CurrentValue;
                }
            }
            if (is_numeric($this->RowIndex)) {
                RenderUploadField($this->gambar, $this->RowIndex);
            }
            ...
            if (!EmptyValue($this->video->CurrentValue)) {
                if ($this->RowIndex == '$rowindex$') {
                    $this->video->Upload->FileName = "";
                } else {
                    $this->video->Upload->FileName = $this->video->CurrentValue;
                }
            }
            if (is_numeric($this->RowIndex)) {
                RenderUploadField($this->video, $this->RowIndex);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->komentar->Required) {
            if (!$this->komentar->IsDetailKey && EmptyValue($this->komentar->FormValue)) {
                $this->komentar->addErrorMessage(str_replace("%s", $this->komentar->caption(), $this->komentar->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // Check referential integrity for master table 'berita'
        $detailKeys = [];
        $keyValue = $rsnew['pid'] ?? $rsold['pid'];
        $detailKeys['pid'] = $keyValue;
        $masterTable = Container("berita");
        ...
        if (!$validMasterRecord) {
            $relatedRecordMsg = str_replace("%t", "berita", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "berita") {
        ...
        // Check referential integrity for master table 'komentar'
