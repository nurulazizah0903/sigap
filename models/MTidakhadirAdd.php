 ...
 * Page class
 */
class MTidakhadirAdd extends MTidakhadir
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MTidakhadirAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("MTidakhadirList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "MTidakhadirList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "MTidakhadirView") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->perjam_value->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->perjam_value->setFormValue($val, true, $validate);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // perjam_value
        $this->perjam_value->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // perjam_value
            $this->perjam_value->setupEditAttributes();
            ...
            if (strval($this->perjam_value->EditValue) != "" && is_numeric($this->perjam_value->EditValue)) {
                $this->perjam_value->EditValue = FormatNumber($this->perjam_value->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->perjam_value->Required) {
            if (!$this->perjam_value->IsDetailKey && EmptyValue($this->perjam_value->FormValue)) {
                $this->perjam_value->addErrorMessage(str_replace("%s", $this->perjam_value->caption(), $this->perjam_value->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->perjam_value->FormValue)) {
            $this->perjam_value->addErrorMessage($this->perjam_value->getErrorMessage(false));
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("MTidakhadirList"), "", $this->TableVar, true);
