 ...
 * Page class
 */
class AbsenDetilList extends AbsenDetil
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AbsenDetilList";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "AbsenDetilDelete";
        $this->MultiUpdateUrl = "AbsenDetilUpdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->cuti); // cuti
            $this->updateSort($this->dinas_luar); // dinas_luar
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fabsen_detillist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fabsen_detillist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("masuk", $this->createColumnOption("masuk"));
            $option->add("absen", $this->createColumnOption("absen"));
            $option->add("ijin", $this->createColumnOption("ijin"));
            $option->add("cuti", $this->createColumnOption("cuti"));
            $option->add("dinas_luar", $this->createColumnOption("dinas_luar"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fabsen_detilsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fabsen_detilsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fabsen_detillist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
