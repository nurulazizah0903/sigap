...
class UserlevelsAdd extends Userlevels
{
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->userlevelid->ViewValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        ...
                        $this->userlevelid->ViewValue = FormatNumber($this->userlevelid->CurrentValue, $this->userlevelid->formatPattern());
                    }
                }
            } else {
            ...
            $curVal = strval($this->userlevelid->CurrentValue);
            if ($curVal != "") {
                $this->userlevelid->EditValue = $this->userlevelid->lookupCacheOption($curVal);
                if ($this->userlevelid->EditValue === null) { // Lookup from database
                    $filterWrk = "`proyek`" . SearchString("=", $curVal, DATATYPE_STRING, "");
                    $sqlWrk = $this->userlevelid->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    ...
                    if ($ari > 0) { // Lookup values found
                        ...
                        $this->userlevelid->EditValue = HtmlEncode(FormatNumber($this->userlevelid->CurrentValue, $this->userlevelid->formatPattern()));
                    }
                }
            } else {
