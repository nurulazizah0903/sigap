...
class Gajitk extends DbTable
{
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "gajitk_detil") {
            $detailUrl = Container("gajitk_detil")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "GajitkList";
        }
        return $detailUrl;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("gajitk_detil")->loadRs("`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("gajitk_detil")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("gajitk_detil")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("gajitk_detil")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("GajitkList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "GajitkEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "GajitkAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("GajitkView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("GajitkView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "GajitkAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "GajitkAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("GajitkEdit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("GajitkEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("GajitkAdd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("GajitkAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("GajitkDelete", $this->getUrlParm());
    }
    ...
    public function renderEditRow()
    {
        ...
        if (strval($this->bulan->EditValue) != "" && is_numeric($this->bulan->EditValue)) {
            $this->bulan->EditValue = FormatNumber($this->bulan->EditValue, null);
        }
