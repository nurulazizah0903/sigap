 ...
 * Page class
 */
class GajiPokokList extends GajiPokok
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "GajiPokokList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "GajiPokokAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "GajiPokokDelete";
        $this->MultiUpdateUrl = "GajiPokokUpdate";
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->ijazah_id);
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->ijazah_id->AdvancedSearch->toJson(), ","); // Field ijazah_id
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fgaji_pokoksrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field ijazah_id
        $this->ijazah_id->AdvancedSearch->SearchValue = @$filter["x_ijazah_id"];
        $this->ijazah_id->AdvancedSearch->SearchOperator = @$filter["z_ijazah_id"];
        $this->ijazah_id->AdvancedSearch->SearchCondition = @$filter["v_ijazah_id"];
        $this->ijazah_id->AdvancedSearch->SearchValue2 = @$filter["y_ijazah_id"];
        $this->ijazah_id->AdvancedSearch->SearchOperator2 = @$filter["w_ijazah_id"];
        $this->ijazah_id->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->ijazah_id); // ijazah_id
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fgaji_pokoklist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fgaji_pokoklist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("ijazah_id", $this->createColumnOption("ijazah_id"));
            $option->add("lama_kerja", $this->createColumnOption("lama_kerja"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fgaji_pokoksrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fgaji_pokoksrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fgaji_pokoklist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->ijazah_id->setDbValue($row['ijazah_id']);
    ...
    protected function newRow()
    {
        ...
        $row['ijazah_id'] = $this->ijazah_id->DefaultValue;
        $row['lama_kerja'] = $this->lama_kerja->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // ijazah_id
            $this->ijazah_id->ViewValue = $this->ijazah_id->CurrentValue;
            $curVal = strval($this->ijazah_id->CurrentValue);
            if ($curVal != "") {
                $this->ijazah_id->ViewValue = $this->ijazah_id->lookupCacheOption($curVal);
                if ($this->ijazah_id->ViewValue === null) { // Lookup from database
                    $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                    $sqlWrk = $this->ijazah_id->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->ijazah_id->Lookup->renderViewRow($rswrk[0]);
                        $this->ijazah_id->ViewValue = $this->ijazah_id->displayValue($arwrk);
                    } else {
                        $this->ijazah_id->ViewValue = FormatNumber($this->ijazah_id->CurrentValue, $this->ijazah_id->formatPattern());
                    }
                }
            } else {
                $this->ijazah_id->ViewValue = null;
            }
            $this->ijazah_id->ViewCustomAttributes = "";

            // lama_kerja
            $this->lama_kerja->ViewValue = $this->lama_kerja->CurrentValue;
            $this->lama_kerja->ViewValue = FormatNumber($this->lama_kerja->ViewValue, $this->lama_kerja->formatPattern());
            ...
            // ijazah_id
            $this->ijazah_id->LinkCustomAttributes = "";
            $this->ijazah_id->HrefValue = "";
            $this->ijazah_id->TooltipValue = "";
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fgaji_pokoksrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
