 ...
 * Page class
 */
class PenempatanList extends Penempatan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PenempatanList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "PenempatanAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "PenempatanDelete";
        $this->MultiUpdateUrl = "PenempatanUpdate";
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->project);
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fpenempatansrch", $filters);
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fpenempatanlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fpenempatanlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("project", $this->createColumnOption("project"));
            $option->add("jabatan", $this->createColumnOption("jabatan"));
            $option->add("tgl_mulai", $this->createColumnOption("tgl_mulai"));
            $option->add("tgl_akhir", $this->createColumnOption("tgl_akhir"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpenempatansrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpenempatansrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpenempatanlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        ...
        $row['project'] = $this->project->DefaultValue;
        $row['jabatan'] = $this->jabatan->DefaultValue;
        $row['tgl_mulai'] = $this->tgl_mulai->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $curVal = strval($this->project->CurrentValue);
            if ($curVal != "") {
                $this->project->ViewValue = $this->project->lookupCacheOption($curVal);
                if ($this->project->ViewValue === null) { // Lookup from database
                    $filterWrk = "`proyek`" . SearchString("=", $curVal, DATATYPE_STRING, "");
                    $sqlWrk = $this->project->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->project->Lookup->renderViewRow($rswrk[0]);
            ...
            if ($curVal != "") {
                ...
                if ($this->jabatan->ViewValue === null) { // Lookup from database
                    $filterWrk = "`nama_jabatan`" . SearchString("=", $curVal, DATATYPE_STRING, "");
            ...
            $this->tgl_mulai->ViewValue = FormatDateTime($this->tgl_mulai->ViewValue, $this->tgl_mulai->formatPattern());
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpenempatansrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
