 ...
 * Page class
 */
class GajismpDelete extends Gajismp
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "GajismpDelete";
    ...
    public function run()
    {
        ...
        if ($filter == "") {
            $this->terminate("GajismpList"); // Prevent SQL injection, return to list
            return;
        }
        ...
        if ($this->isShow()) { // Load records for display
            ...
            if ($this->TotalRecords <= 0) { // No record found, exit
                ...
                $this->terminate("GajismpList"); // Return to list
                return;
            }
        }
    ...
    protected function deleteRows()
    {
        ...
        foreach ($rows as $row) {
            $rsdetail = Container("gajismp_detil")->loadRs("`pid` = " . QuotedValue($row['id'], DATATYPE_NUMBER, 'DB'))->fetch();
            if ($rsdetail !== false) {
                $relatedRecordMsg = str_replace("%t", "gajismp_detil", $Language->phrase("RelatedRecordExists"));
