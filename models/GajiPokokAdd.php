 ...
 * Page class
 */
class GajiPokokAdd extends GajiPokok
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "GajiPokokAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("GajiPokokList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "GajiPokokList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "GajiPokokView") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'ijazah_id' first before field var 'x_ijazah_id'
        $val = $CurrentForm->hasValue("ijazah_id") ? $CurrentForm->getValue("ijazah_id") : $CurrentForm->getValue("x_ijazah_id");
        if (!$this->ijazah_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ijazah_id->Visible = false; // Disable update for API request
            } else {
                $this->ijazah_id->setFormValue($val, true, $validate);
            }
        }
        ...
        if (!$this->lama_kerja->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->lama_kerja->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->ijazah_id->CurrentValue = $this->ijazah_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // ijazah_id
        $this->ijazah_id->RowCssClass = "row";

        // lama_kerja
        $this->lama_kerja->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // ijazah_id
            $this->ijazah_id->setupEditAttributes();
            $this->ijazah_id->EditCustomAttributes = "";
            $this->ijazah_id->EditValue = HtmlEncode($this->ijazah_id->CurrentValue);
            $curVal = strval($this->ijazah_id->CurrentValue);
            if ($curVal != "") {
                $this->ijazah_id->EditValue = $this->ijazah_id->lookupCacheOption($curVal);
                if ($this->ijazah_id->EditValue === null) { // Lookup from database
                    $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                    $sqlWrk = $this->ijazah_id->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->ijazah_id->Lookup->renderViewRow($rswrk[0]);
                        $this->ijazah_id->EditValue = $this->ijazah_id->displayValue($arwrk);
                    } else {
                        $this->ijazah_id->EditValue = HtmlEncode(FormatNumber($this->ijazah_id->CurrentValue, $this->ijazah_id->formatPattern()));
                    }
                }
            } else {
                $this->ijazah_id->EditValue = null;
            }
            $this->ijazah_id->PlaceHolder = RemoveHtml($this->ijazah_id->caption());

            // lama_kerja
            $this->lama_kerja->setupEditAttributes();
            ...
            if (strval($this->lama_kerja->EditValue) != "" && is_numeric($this->lama_kerja->EditValue)) {
                $this->lama_kerja->EditValue = FormatNumber($this->lama_kerja->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->ijazah_id->Required) {
            if (!$this->ijazah_id->IsDetailKey && EmptyValue($this->ijazah_id->FormValue)) {
                $this->ijazah_id->addErrorMessage(str_replace("%s", $this->ijazah_id->caption(), $this->ijazah_id->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->ijazah_id->FormValue)) {
            $this->ijazah_id->addErrorMessage($this->ijazah_id->getErrorMessage(false));
        }
        if ($this->lama_kerja->Required) {
            if (!$this->lama_kerja->IsDetailKey && EmptyValue($this->lama_kerja->FormValue)) {
                $this->lama_kerja->addErrorMessage(str_replace("%s", $this->lama_kerja->caption(), $this->lama_kerja->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->lama_kerja->FormValue)) {
            $this->lama_kerja->addErrorMessage($this->lama_kerja->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // ijazah_id
        $this->ijazah_id->setDbValueDef($rsnew, $this->ijazah_id->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("GajiPokokList"), "", $this->TableVar, true);
