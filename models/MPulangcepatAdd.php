 ...
 * Page class
 */
class MPulangcepatAdd extends MPulangcepat
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MPulangcepatAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("MPulangcepatList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "MPulangcepatList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "MPulangcepatView") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->perjam->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->perjam->setFormValue($val, true, $validate);
            }
        }
        ...
        if (!$this->perhari->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->perhari->setFormValue($val, true, $validate);
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['perjam'] = $this->perjam->DefaultValue;
        $row['perhari'] = $this->perhari->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        // perjam
        $this->perjam->RowCssClass = "row";

        // perhari
        $this->perhari->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->perjam->ViewValue = FormatNumber($this->perjam->ViewValue, $this->perjam->formatPattern());
            $this->perjam->ViewCustomAttributes = "";

            // perhari
            $this->perhari->ViewValue = $this->perhari->CurrentValue;
            $this->perhari->ViewValue = FormatNumber($this->perhari->ViewValue, $this->perhari->formatPattern());
            ...
            // perjam
            $this->perjam->setupEditAttributes();
            ...
            if (strval($this->perjam->EditValue) != "" && is_numeric($this->perjam->EditValue)) {
                $this->perjam->EditValue = FormatNumber($this->perjam->EditValue, null);
            }

            // perhari
            $this->perhari->setupEditAttributes();
            ...
            if (strval($this->perhari->EditValue) != "" && is_numeric($this->perhari->EditValue)) {
                $this->perhari->EditValue = FormatNumber($this->perhari->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->perjam->Required) {
            if (!$this->perjam->IsDetailKey && EmptyValue($this->perjam->FormValue)) {
                $this->perjam->addErrorMessage(str_replace("%s", $this->perjam->caption(), $this->perjam->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->perjam->FormValue)) {
            $this->perjam->addErrorMessage($this->perjam->getErrorMessage(false));
        }
        if ($this->perhari->Required) {
            if (!$this->perhari->IsDetailKey && EmptyValue($this->perhari->FormValue)) {
                $this->perhari->addErrorMessage(str_replace("%s", $this->perhari->caption(), $this->perhari->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->perhari->FormValue)) {
            $this->perhari->addErrorMessage($this->perhari->getErrorMessage(false));
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("MPulangcepatList"), "", $this->TableVar, true);
