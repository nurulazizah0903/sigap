 ...
 * Page class
 */
class PenempatanEdit extends Penempatan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PenempatanEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("PenempatanList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "PenempatanList") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->tgl_mulai->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->tgl_mulai->setFormValue($val, true, $validate);
            }
            $this->tgl_mulai->CurrentValue = UnFormatDateTime($this->tgl_mulai->CurrentValue, $this->tgl_mulai->formatPattern());
        }
    ...
    public function renderRow()
    {
        ...
        // project
        $this->project->RowCssClass = "row";

        // jabatan
        $this->jabatan->RowCssClass = "row";

        // tgl_mulai
        $this->tgl_mulai->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // project
            $this->project->setupEditAttributes();
            $this->project->EditCustomAttributes = "";
            if (!$this->project->Raw) {
                $this->project->CurrentValue = HtmlDecode($this->project->CurrentValue);
            }
            $this->project->EditValue = HtmlEncode($this->project->CurrentValue);
            $curVal = strval($this->project->CurrentValue);
            if ($curVal != "") {
                $this->project->EditValue = $this->project->lookupCacheOption($curVal);
                if ($this->project->EditValue === null) { // Lookup from database
                    $filterWrk = "`proyek`" . SearchString("=", $curVal, DATATYPE_STRING, "");
                    $sqlWrk = $this->project->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->project->Lookup->renderViewRow($rswrk[0]);
                        $this->project->EditValue = $this->project->displayValue($arwrk);
                    } else {
                        $this->project->EditValue = HtmlEncode($this->project->CurrentValue);
                    }
                }
            } else {
                $this->project->EditValue = null;
            }
            ...
            // tgl_mulai
            $this->tgl_mulai->setupEditAttributes();
            $this->tgl_mulai->EditCustomAttributes = "";
            $this->tgl_mulai->EditValue = HtmlEncode(FormatDateTime($this->tgl_mulai->CurrentValue, $this->tgl_mulai->formatPattern()));
    ...
    protected function validateForm()
    {
        ...
        if ($this->tgl_mulai->Required) {
            if (!$this->tgl_mulai->IsDetailKey && EmptyValue($this->tgl_mulai->FormValue)) {
                $this->tgl_mulai->addErrorMessage(str_replace("%s", $this->tgl_mulai->caption(), $this->tgl_mulai->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->tgl_mulai->FormValue, $this->tgl_mulai->formatPattern())) {
            $this->tgl_mulai->addErrorMessage($this->tgl_mulai->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        // tgl_mulai
        $this->tgl_mulai->setDbValueDef($rsnew, UnFormatDateTime($this->tgl_mulai->CurrentValue, $this->tgl_mulai->formatPattern()), null, $this->tgl_mulai->ReadOnly);

        // tgl_akhir
        $this->tgl_akhir->setDbValueDef($rsnew, UnFormatDateTime($this->tgl_akhir->CurrentValue, $this->tgl_akhir->formatPattern()), null, $this->tgl_akhir->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PenempatanList"), "", $this->TableVar, true);
        $pageId = "edit";
