 ...
 * Page class
 */
class GajitkDetilGrid extends GajitkDetil
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "GajitkDetilGrid";
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_voucher") &&
            $CurrentForm->hasValue("o_voucher") &&
            $this->voucher->CurrentValue != $this->voucher->DefaultValue &&
            !($this->voucher->IsForeignKey && $this->getCurrentMasterTable() != "" &&
            $this->voucher->CurrentValue == $this->voucher->getSessionValue())
        ) {
            return false;
        }
        if (
    ...
    public function resetFormError()
    {
        ...
        $this->voucher->clearErrorMessage();
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->voucher->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->voucher->setFormValue($val, true, $validate);
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // voucher
            $this->voucher->setupEditAttributes();
            ...
            if (strval($this->voucher->EditValue) != "" && is_numeric($this->voucher->EditValue)) {
                $this->voucher->EditValue = FormatNumber($this->voucher->EditValue, null);
                $this->voucher->OldValue = $this->voucher->EditValue;
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->voucher->Required) {
            if (!$this->voucher->IsDetailKey && EmptyValue($this->voucher->FormValue)) {
                $this->voucher->addErrorMessage(str_replace("%s", $this->voucher->caption(), $this->voucher->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->voucher->FormValue)) {
            $this->voucher->addErrorMessage($this->voucher->getErrorMessage(false));
        }
