...
class Jabatan extends DbTable
{
    ...
    public function __construct()
    {
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->jenjang->Lookup = new Lookup('jenjang', 'tpendidikan', false, 'id', ["name","","",""], [], [], [], [], [], [], '', '', "`name`");
                break;
            default:
                $this->jenjang->Lookup = new Lookup('jenjang', 'tpendidikan', false, 'id', ["name","","",""], [], [], [], [], [], [], '', '', "`name`");
                break;
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "gajitunjangan") {
            $detailUrl = Container("gajitunjangan")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "JabatanList";
        }
        return $detailUrl;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("gajitunjangan")->loadRs("`pidjabatan` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("gajitunjangan")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("gajitunjangan")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("gajitunjangan")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        $dtlrows = Container("gajitunjangan")->loadRs("`pidjabatan` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("gajitunjangan")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("gajitunjangan")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("gajitunjangan")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getEditUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("JabatanEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("JabatanAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->c_date->EditValue = FormatDateTime($this->c_date->CurrentValue, $this->c_date->formatPattern());
