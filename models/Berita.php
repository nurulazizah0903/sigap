...
class Berita extends DbTable
{
    ...
    public function __construct()
    {
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->grup->Lookup = new Lookup('grup', 'jenis_grup_berita', false, 'nama', ["nama","","",""], [], [], [], [], [], [], '', '', "`nama`");
                break;
            default:
                $this->grup->Lookup = new Lookup('grup', 'jenis_grup_berita', false, 'nama', ["nama","","",""], [], [], [], [], [], [], '', '', "`nama`");
                break;
        ...
        $this->video->UploadAllowedFileExt = "Mp4";
        $this->Fields['video'] = &$this->video;

        // berita
        $this->berita = new DbField(
            'berita',
            'berita',
            'x_berita',
            'berita',
        ...
        $this->berita->InputTextType = "text";
        $this->Fields['berita'] = &$this->berita;

        // gambar
        $this->gambar = new DbField(
            'berita',
            'berita',
            'x_gambar',
            'gambar',
        ...
        $this->gambar->InputTextType = "text";
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "komentar") {
            $detailUrl = Container("komentar")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "BeritaList";
        }
        return $detailUrl;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("komentar")->loadRs("`pid` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("komentar")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("komentar")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("komentar")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        $dtlrows = Container("komentar")->loadRs("`pid` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("komentar")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("komentar")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("komentar")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function deleteUploadedFiles($row)
    {
        ...
        $oldFiles = EmptyValue($row['video']) ? [] : [$row['video']];
        foreach ($oldFiles as $oldFile) {
            if (file_exists($this->video->oldPhysicalUploadPath() . $oldFile)) {
    ...
    public function getEditUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("BeritaEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("BeritaAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
