 ...
 * Page class
 */
class GajismaList extends Gajisma
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "GajismaList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "GajismaAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "GajismaDelete";
        $this->MultiUpdateUrl = "GajismaUpdate";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fgajismasrch", $filters);
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->bulan->Expression . " DESC" . ", " . $this->tahun->Expression . " DESC"; // Set up default sort
    ...
    protected function setupListOptions()
    {
        ...
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'gajisma_detil');
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fgajismalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fgajismalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'gajisma_detil')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("GajismaDetilList?" . Config("TABLE_SHOW_MASTER") . "=gajisma&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("GajismaDetilGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'gajisma')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'gajisma')) {
            ...
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'gajisma')) {
    ...
    protected function renderListOptionsExt()
    {
        ...
        if ($this->DetailPages && $this->DetailPages["gajisma_detil"] && $this->DetailPages["gajisma_detil"]->Visible && $Security->allowList(CurrentProjectID() . 'gajisma_detil')) {
            $link = "";
            $option = $this->ListOptions["detail_gajisma_detil"];
            $url = "GajismaDetilPreview?t=gajisma&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"gajisma_detil\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'gajisma')) {
                $label = $Language->TablePhrase("gajisma_detil", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"gajisma_detil\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("GajismaDetilList?" . Config("TABLE_SHOW_MASTER") . "=gajisma&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
                $title = $Language->TablePhrase("gajisma_detil", "TblCaption");
            ...
            $detailPageObj = Container("GajismaDetilGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'gajisma')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=gajisma_detil"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($url) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'gajisma')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=gajisma_detil"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($url) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'gajisma')) {
                $caption = $Language->phrase("MasterDetailCopyLink");
                $url = GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=gajisma_detil"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($url) . "\">" . $caption . "</a>";
            }
            $btngrp .= "</div>";
            if ($link != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                ...
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'gajisma') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("tahun", $this->createColumnOption("tahun"));
            $option->add("bulan", $this->createColumnOption("bulan"));
            $option->add("tingkat", $this->createColumnOption("tingkat"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fgajismasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fgajismasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fgajismalist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->tahun->ViewValue = FormatNumber($this->tahun->ViewValue, $this->tahun->formatPattern());
            ...
            if ($curVal != "") {
                ...
                if ($this->bulan->ViewValue === null) { // Lookup from database
                    ...
                    $sqlWrk = $this->bulan->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->bulan->Lookup->renderViewRow($rswrk[0]);
                        $this->bulan->ViewValue = $this->bulan->displayValue($arwrk);
                    } else {
                        $this->bulan->ViewValue = FormatNumber($this->bulan->CurrentValue, $this->bulan->formatPattern());
                    }
                }
            } else {
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fgajismasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
