...
class IjinEdit extends Ijin
{
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("IjinList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "IjinList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'tgl_ijin_awal' first before field var 'x_tgl_ijin_awal'
        $val = $CurrentForm->hasValue("tgl_ijin_awal") ? $CurrentForm->getValue("tgl_ijin_awal") : $CurrentForm->getValue("x_tgl_ijin_awal");
        if (!$this->tgl_ijin_awal->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tgl_ijin_awal->Visible = false; // Disable update for API request
            } else {
                $this->tgl_ijin_awal->setFormValue($val, true, $validate);
            }
            $this->tgl_ijin_awal->CurrentValue = UnFormatDateTime($this->tgl_ijin_awal->CurrentValue, $this->tgl_ijin_awal->formatPattern());
        }

        // Check field name 'tgl_ijin_akhir' first before field var 'x_tgl_ijin_akhir'
        $val = $CurrentForm->hasValue("tgl_ijin_akhir") ? $CurrentForm->getValue("tgl_ijin_akhir") : $CurrentForm->getValue("x_tgl_ijin_akhir");
        if (!$this->tgl_ijin_akhir->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tgl_ijin_akhir->Visible = false; // Disable update for API request
            } else {
                $this->tgl_ijin_akhir->setFormValue($val, true, $validate);
            }
            $this->tgl_ijin_akhir->CurrentValue = UnFormatDateTime($this->tgl_ijin_akhir->CurrentValue, $this->tgl_ijin_akhir->formatPattern());
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->tgl->CurrentValue = UnFormatDateTime($this->tgl->CurrentValue, $this->tgl->formatPattern());
        $this->tgl_ijin_awal->CurrentValue = $this->tgl_ijin_awal->FormValue;
        $this->tgl_ijin_awal->CurrentValue = UnFormatDateTime($this->tgl_ijin_awal->CurrentValue, $this->tgl_ijin_awal->formatPattern());
        $this->tgl_ijin_akhir->CurrentValue = $this->tgl_ijin_akhir->FormValue;
        $this->tgl_ijin_akhir->CurrentValue = UnFormatDateTime($this->tgl_ijin_akhir->CurrentValue, $this->tgl_ijin_akhir->formatPattern());
    ...
    protected function newRow()
    {
        ...
        $row['tgl_ijin_awal'] = $this->tgl_ijin_awal->DefaultValue;
        $row['tgl_ijin_akhir'] = $this->tgl_ijin_akhir->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        // tgl_ijin_awal
        $this->tgl_ijin_awal->RowCssClass = "row";

        // tgl_ijin_akhir
        $this->tgl_ijin_akhir->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->tgl_ijin_awal->EditValue = HtmlEncode(FormatDateTime($this->tgl_ijin_awal->CurrentValue, $this->tgl_ijin_awal->formatPattern()));
            ...
            $this->tgl_ijin_akhir->EditValue = HtmlEncode(FormatDateTime($this->tgl_ijin_akhir->CurrentValue, $this->tgl_ijin_akhir->formatPattern()));
    ...
    protected function validateForm()
    {
        ...
        if ($this->tgl_ijin_awal->Required) {
            if (!$this->tgl_ijin_awal->IsDetailKey && EmptyValue($this->tgl_ijin_awal->FormValue)) {
                $this->tgl_ijin_awal->addErrorMessage(str_replace("%s", $this->tgl_ijin_awal->caption(), $this->tgl_ijin_awal->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->tgl_ijin_awal->FormValue, $this->tgl_ijin_awal->formatPattern())) {
            $this->tgl_ijin_awal->addErrorMessage($this->tgl_ijin_awal->getErrorMessage(false));
        }
        if ($this->tgl_ijin_akhir->Required) {
            if (!$this->tgl_ijin_akhir->IsDetailKey && EmptyValue($this->tgl_ijin_akhir->FormValue)) {
                $this->tgl_ijin_akhir->addErrorMessage(str_replace("%s", $this->tgl_ijin_akhir->caption(), $this->tgl_ijin_akhir->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->tgl_ijin_akhir->FormValue, $this->tgl_ijin_akhir->formatPattern())) {
            $this->tgl_ijin_akhir->addErrorMessage($this->tgl_ijin_akhir->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        // tgl_ijin_awal
        $this->tgl_ijin_awal->setDbValueDef($rsnew, UnFormatDateTime($this->tgl_ijin_awal->CurrentValue, $this->tgl_ijin_awal->formatPattern()), null, $this->tgl_ijin_awal->ReadOnly);

        // tgl_ijin_akhir
        $this->tgl_ijin_akhir->setDbValueDef($rsnew, UnFormatDateTime($this->tgl_ijin_akhir->CurrentValue, $this->tgl_ijin_akhir->formatPattern()), null, $this->tgl_ijin_akhir->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("IjinList"), "", $this->TableVar, true);
        $pageId = "edit";
