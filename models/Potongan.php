...
class Potongan extends DbTable
{
    ...
    public function __construct()
    {
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->nama->Lookup = new Lookup('nama', 'pegawai', false, 'nip', ["nama","","",""], [], [], [], [], ["jabatan","jenjang_id"], ["x_jabatan_id","x_jenjang_id"], '', '', "`nama`");
                break;
            default:
                $this->nama->Lookup = new Lookup('nama', 'pegawai', false, 'nip', ["nama","","",""], [], [], [], [], ["jabatan","jenjang_id"], ["x_jabatan_id","x_jenjang_id"], '', '', "`nama`");
                break;
        ...
        $this->izin->Required = true; // Required field
        ...
        $this->pulcep->Required = true; // Required field
        ...
        $this->tidakhadirjam->Required = true; // Required field
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("PotonganList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "PotonganEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "PotonganAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("PotonganView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("PotonganView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "PotonganAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "PotonganAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("PotonganEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PotonganAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("PotonganDelete", $this->getUrlParm());
    }
    ...
    public function rowInserting($rsold, &$rsnew)
    {
        ...
        // To cancel, set return value to false
            if ($this->jenjang_id->CurrentValue == '1')
                                ...
                                $terlambat = ExecuteRow("SELECT * from terlambat where jenjang_id ='1' AND jenjang_id ='1'");
                                ...
                                $rsnew["value_terlambat"] = $terlambat["value"];
                                    $rsnew["total"] = ($this->izin->CurrentValue * $tidakhadir["value"]) + ($this->sakit->CurrentValue * $tidakhadir["value"]) + ($this->terlambat->CurrentValue * $terlambat["value"]) + ($this->tidakhadir->CurrentValue * $tidakhadir["value"]) + ($this->sakitperjam->CurrentValue * $sakit["perjam_value"]) + ($this->pulcep->CurrentValue * $pulcep["perhari"])+ ($this->izinperjam->CurrentValue * $tidakhadir["perjam_value"]);
                                } else {
                                $tidakhadir2 = ExecuteRow("SELECT * from m_tidakhadir where jenjang_id='1' AND jabatan_id='2'");
                                $ijin2 = ExecuteRow("SELECT * from jenis_ijin where jabatan_id ='2' AND jenjang_id ='1'");
                                $terlambat2 = ExecuteRow("SELECT * from terlambat where jenjang_id ='1' AND jabatan_id ='2'");
                                    $rsnew["value_terlambat"] = $terlambat2["value"];
                                    $rsnew["total"] = ($this->izin->CurrentValue * $wsquery3["value"]) + ($this->sakit->CurrentValue * $tidakhadir2["value"]) + ($this->terlambat->CurrentValue * $terlambat2["value"]) + ($this->tidakhadir->CurrentValue * $wsquery7["value"]) + ($this->sakitperjam->CurrentValue * $tidakhadir2["perjam_value"]) + ($this->pulcep->CurrentValue * $wsquery3["value_pulcep"]);
                                }
            return true;
    }
    ...
    /*        $wsquery2 = ExecuteRow("SELECT * from terlambat");
            $wsquery3 = ExecuteRow("SELECT * from jenis_ijin where id ='1'");
            $wsquery4 = ExecuteRow("SELECT * from jenis_ijin where id ='2'");
            $wsquery7 = ExecuteRow("SELECT * from m_tidakhadir");
            $this->total->ViewValue = ($this->izin->CurrentValue * $wsquery3["value"]) + ($this->sakit->CurrentValue * $wsquery4["value"]) + ($this->terlambat->CurrentValue * $wsquery2["value"]) + ($this->tidakhadir->CurrentValue * $wsquery7["value"]);
    */}
