...
<div id="xsr_<?php echo $m_sma_list->SearchRowCount ?>" class="ew-row d-sm-flex">
	...
	<div id="xsc_bulan" class="ew-cell form-group">
		<label for="x_bulan" class="ew-search-caption ew-label"><?php echo $m_sma_list->bulan->caption() ?></label>
		<span class="ew-search-operator">
<?php echo $Language->phrase("=") ?>
...
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_bulan"><?php echo EmptyValue(strval($m_sma_list->bulan->AdvancedSearch->ViewValue)) ? $Language->phrase("PleaseSelect") : $m_sma_list->bulan->AdvancedSearch->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($m_sma_list->bulan->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($m_sma_list->bulan->ReadOnly || $m_sma_list->bulan->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_bulan',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $m_sma_list->bulan->Lookup->getParamTag($m_sma_list, "p_x_bulan") ?>
<input type="hidden" data-table="m_sma" data-field="x_bulan" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $m_sma_list->bulan->displayValueSeparatorAttribute() ?>" name="x_bulan" id="x_bulan" value="<?php echo $m_sma_list->bulan->AdvancedSearch->SearchValue ?>"<?php echo $m_sma_list->bulan->editAttributes() ?>>
</span>
	</div>
	<?php if ($m_sma_list->SearchColumnCount % $m_sma_list->SearchFieldsPerRow == 0) { ?>
</div>
...
?>
<?php if ($m_sma_list->createby->Visible) { // createby ?>
	<?php if ($m_sma_list->SortUrl($m_sma_list->createby) == "") { ?>
		<th data-name="createby" class="<?php echo $m_sma_list->createby->headerCellClass() ?>"><div id="elh_m_sma_createby" class="m_sma_createby"><div class="ew-table-header-caption"><?php echo $m_sma_list->createby->caption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="createby" class="<?php echo $m_sma_list->createby->headerCellClass() ?>"><div class="ew-pointer" onclick="ew.sort(event, '<?php echo $m_sma_list->SortUrl($m_sma_list->createby) ?>', 1);"><div id="elh_m_sma_createby" class="m_sma_createby">
			<div class="ew-table-header-btn"><span class="ew-table-header-caption"><?php echo $m_sma_list->createby->caption() ?></span><span class="ew-table-header-sort"><?php if ($m_sma_list->createby->getSort() == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($m_sma_list->createby->getSort() == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($m_sma_list->tahun->Visible) { // tahun ?>
...
?>
	<?php if ($m_sma_list->createby->Visible) { // createby ?>
		<td data-name="createby" <?php echo $m_sma_list->createby->cellAttributes() ?>>
<span id="el<?php echo $m_sma_list->RowCount ?>_m_sma_createby">
<span<?php echo $m_sma_list->createby->viewAttributes() ?>><?php echo $m_sma_list->createby->getViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($m_sma_list->tahun->Visible) { // tahun ?>
