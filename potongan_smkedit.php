...
loadjs.ready("head", function() {
	...
	fpotongan_smkedit.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
			<?php if ($potongan_smk_edit->month->Required) { ?>
				elm = this.getElements("x" + infix + "_month");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_smk_edit->month->caption(), $potongan_smk_edit->month->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_month");
				if (elm && !ew.checkDateDef(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_smk_edit->month->errorMessage()) ?>");
...
<?php } ?>
<?php if ($potongan_smk_edit->month->Visible) { // month ?>
	<div id="r_month" class="form-group row">
		<label id="elh_potongan_smk_month" for="x_month" class="<?php echo $potongan_smk_edit->LeftColumnClass ?>"><?php echo $potongan_smk_edit->month->caption() ?><?php echo $potongan_smk_edit->month->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_smk_edit->RightColumnClass ?>"><div <?php echo $potongan_smk_edit->month->cellAttributes() ?>>
<span id="el_potongan_smk_month">
<input type="text" data-table="potongan_smk" data-field="x_month" name="x_month" id="x_month" maxlength="10" placeholder="<?php echo HtmlEncode($potongan_smk_edit->month->getPlaceHolder()) ?>" value="<?php echo $potongan_smk_edit->month->EditValue ?>"<?php echo $potongan_smk_edit->month->editAttributes() ?>>
<?php if (!$potongan_smk_edit->month->ReadOnly && !$potongan_smk_edit->month->Disabled && !isset($potongan_smk_edit->month->EditAttrs["readonly"]) && !isset($potongan_smk_edit->month->EditAttrs["disabled"])) { ?>
<script>
loadjs.ready(["fpotongan_smkedit", "datetimepicker"], function() {
	ew.createDateTimePicker("fpotongan_smkedit", "x_month", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
<?php echo $potongan_smk_edit->month->CustomMsg ?></div></div>
	</div>
<?php } ?>
