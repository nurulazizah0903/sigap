...
loadjs.ready("head", function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
				...
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($m_sakit_edit->jenjang_id->errorMessage()) ?>");
			...
			<?php } ?>
				elm = this.getElements("x" + infix + "_jabatan");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($m_sakit_edit->jabatan->errorMessage()) ?>");
	...
	fm_sakitedit.autoSuggests["x_jenjang_id"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
	...
	fm_sakitedit.autoSuggests["x_jabatan"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
...
<?php if ($m_sakit_edit->jenjang_id->Visible) { // jenjang_id ?>
	<div id="r_jenjang_id" class="form-group row">
		<label id="elh_m_sakit_jenjang_id" class="<?php echo $m_sakit_edit->LeftColumnClass ?>"><?php echo $m_sakit_edit->jenjang_id->caption() ?><?php echo $m_sakit_edit->jenjang_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php
$onchange = $m_sakit_edit->jenjang_id->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$m_sakit_edit->jenjang_id->EditAttrs["onchange"] = "";
?>
<span id="as_x_jenjang_id">
	<div class="input-group">
		<input type="text" class="form-control" name="sv_x_jenjang_id" id="sv_x_jenjang_id" value="<?php echo RemoveHtml($m_sakit_edit->jenjang_id->EditValue) ?>" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($m_sakit_edit->jenjang_id->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($m_sakit_edit->jenjang_id->getPlaceHolder()) ?>"<?php echo $m_sakit_edit->jenjang_id->editAttributes() ?>>
		<div class="input-group-append">
			<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($m_sakit_edit->jenjang_id->caption()), $Language->phrase("LookupLink", TRUE))) ?>" onclick="ew.modalLookupShow({lnk:this,el:'x_jenjang_id',m:0,n:10,srch:true});" class="ew-lookup-btn btn btn-default"<?php echo ($m_sakit_edit->jenjang_id->ReadOnly || $m_sakit_edit->jenjang_id->Disabled) ? " disabled" : "" ?>><i class="fas fa-search ew-icon"></i></button>
		</div>
	</div>
</span>
<input type="hidden" data-table="m_sakit" data-field="x_jenjang_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $m_sakit_edit->jenjang_id->displayValueSeparatorAttribute() ?>" name="x_jenjang_id" id="x_jenjang_id" value="<?php echo HtmlEncode($m_sakit_edit->jenjang_id->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fm_sakitedit"], function() {
	fm_sakitedit.createAutoSuggest({"id":"x_jenjang_id","forceSelect":false});
});
</script>
...
<?php if ($m_sakit_edit->jabatan->Visible) { // jabatan ?>
	<div id="r_jabatan" class="form-group row">
		<label id="elh_m_sakit_jabatan" class="<?php echo $m_sakit_edit->LeftColumnClass ?>"><?php echo $m_sakit_edit->jabatan->caption() ?><?php echo $m_sakit_edit->jabatan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php
$onchange = $m_sakit_edit->jabatan->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$m_sakit_edit->jabatan->EditAttrs["onchange"] = "";
?>
<span id="as_x_jabatan">
	<div class="input-group">
		<input type="text" class="form-control" name="sv_x_jabatan" id="sv_x_jabatan" value="<?php echo RemoveHtml($m_sakit_edit->jabatan->EditValue) ?>" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($m_sakit_edit->jabatan->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($m_sakit_edit->jabatan->getPlaceHolder()) ?>"<?php echo $m_sakit_edit->jabatan->editAttributes() ?>>
		<div class="input-group-append">
			<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($m_sakit_edit->jabatan->caption()), $Language->phrase("LookupLink", TRUE))) ?>" onclick="ew.modalLookupShow({lnk:this,el:'x_jabatan',m:0,n:10,srch:true});" class="ew-lookup-btn btn btn-default"<?php echo ($m_sakit_edit->jabatan->ReadOnly || $m_sakit_edit->jabatan->Disabled) ? " disabled" : "" ?>><i class="fas fa-search ew-icon"></i></button>
		</div>
	</div>
</span>
<input type="hidden" data-table="m_sakit" data-field="x_jabatan" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $m_sakit_edit->jabatan->displayValueSeparatorAttribute() ?>" name="x_jabatan" id="x_jabatan" value="<?php echo HtmlEncode($m_sakit_edit->jabatan->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fm_sakitedit"], function() {
	fm_sakitedit.createAutoSuggest({"id":"x_jabatan","forceSelect":false});
});
</script>
