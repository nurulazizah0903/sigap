...
class barang extends DbTable
{
	...
	public function __construct()
	{
		...
		// Kode_Barang
		$this->Kode_Barang = new DbField('barang', 'barang', 'x_Kode_Barang', 'Kode_Barang', '`Kode_Barang`', '`Kode_Barang`', 200, 3, -1, FALSE, '`Kode_Barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Nama_Barang
		$this->Nama_Barang = new DbField('barang', 'barang', 'x_Nama_Barang', 'Nama_Barang', '`Nama_Barang`', '`Nama_Barang`', 200, 20, -1, FALSE, '`Nama_Barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('Kode_Barang', $row) ? $row['Kode_Barang'] : NULL;
		else
			$val = $this->Kode_Barang->OldValue !== NULL ? $this->Kode_Barang->OldValue : $this->Kode_Barang->CurrentValue;
		if ($val == NULL)
	...
	{
		global $Language;
		if ($pageName == "barangview.php")
			return $Language->phrase("View");
		elseif ($pageName == "barangedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "barangadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("barangview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("barangview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "barangadd.php?" . $this->getUrlParm($parm);
		else
			$url = "barangadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("barangedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("barangadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("barangdelete.php", $this->getUrlParm());
	}
