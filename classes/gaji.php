...
class gaji extends DbTable
{
	...
	function Row_Inserting($rsold, &$rsnew) {
		...
		$querypotongan = ExecuteRow("SELECT * FROM potongan_sma ORDER BY id DESC");
			if ($this->jenis_guru->CurrentValue == '1'){
			 ...
			 $piket= ExecuteRow("select * from m_piket where jenjang='1' and jenis_sertif ='1'");
			 $kehadiran= ExecuteRow("Select * from m_kehadiran where jenjang='1' AND jenis_jabatan='1' AND sertif='1'");	
			 ...
			 $rsnew["tugastambahan"] = $tunjanagan_tambahan["value"];
				...
				$piket= ExecuteRow("select * from m_piket where jenjang='1' and jenis_sertif ='2'");	
				$kehadiran= ExecuteRow("Select * from m_kehadiran where jenjang='1' AND jenis_jabatan='1' AND sertif='2'");	
				$query2 = ExecuteRow("SELECT * FROM jabatan JOIN gajitunjangan ON jabatan.id = gajitunjangan.pidjabatan where gajitunjangan.pidjabatan='9'");
				if ($this->tambahan->CurrentValue == '2'){
					$tunjanagan_tambahan=ExecuteRow("select * from tunjangan_tambahan where jenjang='1' AND kualifikasi='2'");
				}else if ($this->tambahan->CurrentValue == '3'){
	...
	{
	if ($fld->FldName == "user_group")
	{
				 //PUT YOUR CONDITIONS HERE BEORE AND CHOOSE HOW TO APPLY THE FILTER
		 ew_AddFilter($filter, "group_id > '0'");
	}
	*/
	}
	...
	function Row_Rendering() {
		...
		if (CurrentPageID() == "add") {
				...
				$id_user = ExecuteRows("SELECT * FROM pegawai where jenjang_id ='2' AND `type`= '1'");
