...
/**
 * Table class for pengetahuan
 */
class pengetahuan extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'pengetahuan';
		$this->TableName = 'pengetahuan';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`pengetahuan`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('pengetahuan', 'pengetahuan', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// grup
		$this->grup = new DbField('pengetahuan', 'pengetahuan', 'x_grup', 'grup', '`grup`', '`grup`', 200, 255, -1, FALSE, '`grup`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->grup->Sortable = TRUE; // Allow sort
		$this->fields['grup'] = &$this->grup;

		// judul
		$this->judul = new DbField('pengetahuan', 'pengetahuan', 'x_judul', 'judul', '`judul`', '`judul`', 201, 500, -1, FALSE, '`judul`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->judul->Sortable = TRUE; // Allow sort
		$this->fields['judul'] = &$this->judul;

		// isi
		$this->isi = new DbField('pengetahuan', 'pengetahuan', 'x_isi', 'isi', '`isi`', '`isi`', 201, 65535, -1, FALSE, '`isi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->isi->Sortable = TRUE; // Allow sort
		$this->fields['isi'] = &$this->isi;

		// sumber_url
		$this->sumber_url = new DbField('pengetahuan', 'pengetahuan', 'x_sumber_url', 'sumber_url', '`sumber_url`', '`sumber_url`', 200, 255, -1, FALSE, '`sumber_url`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->sumber_url->Sortable = TRUE; // Allow sort
		$this->fields['sumber_url'] = &$this->sumber_url;

		// dokumen
		$this->dokumen = new DbField('pengetahuan', 'pengetahuan', 'x_dokumen', 'dokumen', '`dokumen`', '`dokumen`', 200, 255, -1, FALSE, '`dokumen`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->dokumen->Sortable = TRUE; // Allow sort
		$this->fields['dokumen'] = &$this->dokumen;

		// visual
		$this->visual = new DbField('pengetahuan', 'pengetahuan', 'x_visual', 'visual', '`visual`', '`visual`', 200, 255, -1, FALSE, '`visual`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->visual->Sortable = TRUE; // Allow sort
		$this->fields['visual'] = &$this->visual;

		// c_by
		$this->c_by = new DbField('pengetahuan', 'pengetahuan', 'x_c_by', 'c_by', '`c_by`', '`c_by`', 3, 11, -1, FALSE, '`c_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`pengetahuan`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->sumber_url->DbValue = $row['sumber_url'];
	...
	{
		global $Language;
		if ($pageName == "pengetahuanview.php")
			return $Language->phrase("View");
		elseif ($pageName == "pengetahuanedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "pengetahuanadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("pengetahuanview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("pengetahuanview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "pengetahuanadd.php?" . $this->getUrlParm($parm);
		else
			$url = "pengetahuanadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("pengetahuanedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("pengetahuanadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("pengetahuandelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->sumber_url->setDbValue($rs->fields('sumber_url'));
		$this->dokumen->setDbValue($rs->fields('dokumen'));
	...
	public function renderEditRow()
	{
		...
		// sumber_url
		$this->sumber_url->EditAttrs["class"] = "form-control";
		...
		$this->sumber_url->EditValue = $this->sumber_url->CurrentValue;
		...
		$this->dokumen->PlaceHolder = RemoveHtml($this->dokumen->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->sumber_url);
					$doc->exportCaption($this->dokumen);
					$doc->exportCaption($this->visual);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->sumber_url);
						$doc->exportField($this->dokumen);
						$doc->exportField($this->visual);
