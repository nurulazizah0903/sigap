...
/**
 * Table class for penempatan
 */
class penempatan extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'penempatan';
		$this->TableName = 'penempatan';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`penempatan`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('penempatan', 'penempatan', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// pegawai
		$this->pegawai = new DbField('penempatan', 'penempatan', 'x_pegawai', 'pegawai', '`pegawai`', '`pegawai`', 3, 11, -1, FALSE, '`pegawai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// project
		$this->project = new DbField('penempatan', 'penempatan', 'x_project', 'project', '`project`', '`project`', 200, 255, -1, FALSE, '`project`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->project->Sortable = TRUE; // Allow sort
		$this->fields['project'] = &$this->project;

		// jabatan
		$this->jabatan = new DbField('penempatan', 'penempatan', 'x_jabatan', 'jabatan', '`jabatan`', '`jabatan`', 3, 11, -1, FALSE, '`jabatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tgl_mulai
		$this->tgl_mulai = new DbField('penempatan', 'penempatan', 'x_tgl_mulai', 'tgl_mulai', '`tgl_mulai`', CastDateFieldForLike("`tgl_mulai`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_mulai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tgl_akhir
		$this->tgl_akhir = new DbField('penempatan', 'penempatan', 'x_tgl_akhir', 'tgl_akhir', '`tgl_akhir`', CastDateFieldForLike("`tgl_akhir`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_akhir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`penempatan`";
	}
	...
	{
		global $Language;
		if ($pageName == "penempatanview.php")
			return $Language->phrase("View");
		elseif ($pageName == "penempatanedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "penempatanadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("penempatanview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("penempatanview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "penempatanadd.php?" . $this->getUrlParm($parm);
		else
			$url = "penempatanadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("penempatanedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("penempatanadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("penempatandelete.php", $this->getUrlParm());
	}
