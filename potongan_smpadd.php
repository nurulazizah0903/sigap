...
loadjs.ready("head", function() {
	...
	fpotongan_smpadd.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($potongan_smp_add->month->Required) { ?>
				elm = this.getElements("x" + infix + "_month");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $potongan_smp_add->month->caption(), $potongan_smp_add->month->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_month");
				if (elm && !ew.checkDateDef(elm.value))
					return this.onError(elm, "<?php echo JsEncode($potongan_smp_add->month->errorMessage()) ?>");
...
<?php } ?>
<?php if ($potongan_smp_add->month->Visible) { // month ?>
	<div id="r_month" class="form-group row">
		<label id="elh_potongan_smp_month" for="x_month" class="<?php echo $potongan_smp_add->LeftColumnClass ?>"><?php echo $potongan_smp_add->month->caption() ?><?php echo $potongan_smp_add->month->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $potongan_smp_add->RightColumnClass ?>"><div <?php echo $potongan_smp_add->month->cellAttributes() ?>>
<span id="el_potongan_smp_month">
<input type="text" data-table="potongan_smp" data-field="x_month" name="x_month" id="x_month" maxlength="10" placeholder="<?php echo HtmlEncode($potongan_smp_add->month->getPlaceHolder()) ?>" value="<?php echo $potongan_smp_add->month->EditValue ?>"<?php echo $potongan_smp_add->month->editAttributes() ?>>
<?php if (!$potongan_smp_add->month->ReadOnly && !$potongan_smp_add->month->Disabled && !isset($potongan_smp_add->month->EditAttrs["readonly"]) && !isset($potongan_smp_add->month->EditAttrs["disabled"])) { ?>
<script>
loadjs.ready(["fpotongan_smpadd", "datetimepicker"], function() {
	ew.createDateTimePicker("fpotongan_smpadd", "x_month", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
<?php echo $potongan_smp_add->month->CustomMsg ?></div></div>
	</div>
<?php } ?>
