		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
				...
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($m_karyawan_smk_edit->createby->errorMessage()) ?>");
...
<?php if ($m_karyawan_smk_edit->createby->Visible) { // createby ?>
	<div id="r_createby" class="form-group row">
		<label id="elh_m_karyawan_smk_createby" class="<?php echo $m_karyawan_smk_edit->LeftColumnClass ?>"><?php echo $m_karyawan_smk_edit->createby->caption() ?><?php echo $m_karyawan_smk_edit->createby->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $m_karyawan_smk_edit->RightColumnClass ?>"><div <?php echo $m_karyawan_smk_edit->createby->cellAttributes() ?>>
<span id="el_m_karyawan_smk_createby">
<?php
$onchange = $m_karyawan_smk_edit->createby->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$m_karyawan_smk_edit->createby->EditAttrs["onchange"] = "";
?>
<span id="as_x_createby">
	<input type="text" class="form-control" name="sv_x_createby" id="sv_x_createby" value="<?php echo RemoveHtml($m_karyawan_smk_edit->createby->EditValue) ?>" size="30" maxlength="10" placeholder="<?php echo HtmlEncode($m_karyawan_smk_edit->createby->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($m_karyawan_smk_edit->createby->getPlaceHolder()) ?>"<?php echo $m_karyawan_smk_edit->createby->editAttributes() ?>>
</span>
<input type="hidden" data-table="m_karyawan_smk" data-field="x_createby" data-value-separator="<?php echo $m_karyawan_smk_edit->createby->displayValueSeparatorAttribute() ?>" name="x_createby" id="x_createby" value="<?php echo HtmlEncode($m_karyawan_smk_edit->createby->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fm_karyawan_smkedit"], function() {
	fm_karyawan_smkedit.createAutoSuggest({"id":"x_createby","forceSelect":false});
});
</script>
<?php echo $m_karyawan_smk_edit->createby->Lookup->getParamTag($m_karyawan_smk_edit, "p_x_createby") ?>
</span>
<?php echo $m_karyawan_smk_edit->createby->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_bulan"><?php echo EmptyValue(strval($m_karyawan_smk_edit->bulan->ViewValue)) ? $Language->phrase("PleaseSelect") : $m_karyawan_smk_edit->bulan->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($m_karyawan_smk_edit->bulan->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($m_karyawan_smk_edit->bulan->ReadOnly || $m_karyawan_smk_edit->bulan->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_bulan',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $m_karyawan_smk_edit->bulan->Lookup->getParamTag($m_karyawan_smk_edit, "p_x_bulan") ?>
<input type="hidden" data-table="m_karyawan_smk" data-field="x_bulan" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $m_karyawan_smk_edit->bulan->displayValueSeparatorAttribute() ?>" name="x_bulan" id="x_bulan" value="<?php echo $m_karyawan_smk_edit->bulan->CurrentValue ?>"<?php echo $m_karyawan_smk_edit->bulan->editAttributes() ?>>
</span>
