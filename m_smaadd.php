		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
				...
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($m_sma_add->createby->errorMessage()) ?>");
...
<?php if ($m_sma_add->createby->Visible) { // createby ?>
	<div id="r_createby" class="form-group row">
		<label id="elh_m_sma_createby" class="<?php echo $m_sma_add->LeftColumnClass ?>"><?php echo $m_sma_add->createby->caption() ?><?php echo $m_sma_add->createby->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $m_sma_add->RightColumnClass ?>"><div <?php echo $m_sma_add->createby->cellAttributes() ?>>
<span id="el_m_sma_createby">
<?php
$onchange = $m_sma_add->createby->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$m_sma_add->createby->EditAttrs["onchange"] = "";
?>
<span id="as_x_createby">
	<input type="text" class="form-control" name="sv_x_createby" id="sv_x_createby" value="<?php echo RemoveHtml($m_sma_add->createby->EditValue) ?>" size="30" maxlength="10" placeholder="<?php echo HtmlEncode($m_sma_add->createby->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($m_sma_add->createby->getPlaceHolder()) ?>"<?php echo $m_sma_add->createby->editAttributes() ?>>
</span>
<input type="hidden" data-table="m_sma" data-field="x_createby" data-value-separator="<?php echo $m_sma_add->createby->displayValueSeparatorAttribute() ?>" name="x_createby" id="x_createby" value="<?php echo HtmlEncode($m_sma_add->createby->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fm_smaadd"], function() {
	fm_smaadd.createAutoSuggest({"id":"x_createby","forceSelect":false});
});
</script>
<?php echo $m_sma_add->createby->Lookup->getParamTag($m_sma_add, "p_x_createby") ?>
</span>
<?php echo $m_sma_add->createby->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if ($m_sma_add->bulan->Visible) { // bulan ?>
	<div id="r_bulan" class="form-group row">
		<label id="elh_m_sma_bulan" for="x_bulan" class="<?php echo $m_sma_add->LeftColumnClass ?>"><?php echo $m_sma_add->bulan->caption() ?><?php echo $m_sma_add->bulan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $m_sma_add->RightColumnClass ?>"><div <?php echo $m_sma_add->bulan->cellAttributes() ?>>
<span id="el_m_sma_bulan">
<div class="input-group ew-lookup-list">
	<div class="form-control ew-lookup-text" tabindex="-1" id="lu_x_bulan"><?php echo EmptyValue(strval($m_sma_add->bulan->ViewValue)) ? $Language->phrase("PleaseSelect") : $m_sma_add->bulan->ViewValue ?></div>
	<div class="input-group-append">
		<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($m_sma_add->bulan->caption()), $Language->phrase("LookupLink", TRUE))) ?>" class="ew-lookup-btn btn btn-default"<?php echo ($m_sma_add->bulan->ReadOnly || $m_sma_add->bulan->Disabled) ? " disabled" : "" ?> onclick="ew.modalLookupShow({lnk:this,el:'x_bulan',m:0,n:10});"><i class="fas fa-search ew-icon"></i></button>
	</div>
</div>
<?php echo $m_sma_add->bulan->Lookup->getParamTag($m_sma_add, "p_x_bulan") ?>
<input type="hidden" data-table="m_sma" data-field="x_bulan" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $m_sma_add->bulan->displayValueSeparatorAttribute() ?>" name="x_bulan" id="x_bulan" value="<?php echo $m_sma_add->bulan->CurrentValue ?>"<?php echo $m_sma_add->bulan->editAttributes() ?>>
</span>
