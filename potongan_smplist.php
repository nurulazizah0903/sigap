...
<?php if ($potongan_smp_list->u_by->Visible) { // u_by ?>
	...
	<?php } ?>
<?php } ?>
<?php if ($potongan_smp_list->month->Visible) { // month ?>
	<?php if ($potongan_smp_list->SortUrl($potongan_smp_list->month) == "") { ?>
		<th data-name="month" class="<?php echo $potongan_smp_list->month->headerCellClass() ?>"><div id="elh_potongan_smp_month" class="potongan_smp_month"><div class="ew-table-header-caption"><?php echo $potongan_smp_list->month->caption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="month" class="<?php echo $potongan_smp_list->month->headerCellClass() ?>"><div class="ew-pointer" onclick="ew.sort(event, '<?php echo $potongan_smp_list->SortUrl($potongan_smp_list->month) ?>', 1);"><div id="elh_potongan_smp_month" class="potongan_smp_month">
			<div class="ew-table-header-btn"><span class="ew-table-header-caption"><?php echo $potongan_smp_list->month->caption() ?><?php echo $Language->phrase("SrchLegend") ?></span><span class="ew-table-header-sort"><?php if ($potongan_smp_list->month->getSort() == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($potongan_smp_list->month->getSort() == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<span<?php echo $potongan_smp_list->u_by->viewAttributes() ?>><?php echo $potongan_smp_list->u_by->getViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($potongan_smp_list->month->Visible) { // month ?>
		<td data-name="month" <?php echo $potongan_smp_list->month->cellAttributes() ?>>
<span id="el<?php echo $potongan_smp_list->RowCount ?>_potongan_smp_month">
<span<?php echo $potongan_smp_list->month->viewAttributes() ?>><?php echo $potongan_smp_list->month->getViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($potongan_smp_list->nama->Visible) { // nama ?>
