		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
				...
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($m_karyawan_tk_add->createby->errorMessage()) ?>");
			...
			<?php } ?>
				elm = this.getElements("x" + infix + "_bulan");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($m_karyawan_tk_add->bulan->errorMessage()) ?>");
...
<?php if ($m_karyawan_tk_add->createby->Visible) { // createby ?>
	<div id="r_createby" class="form-group row">
		<label id="elh_m_karyawan_tk_createby" class="<?php echo $m_karyawan_tk_add->LeftColumnClass ?>"><?php echo $m_karyawan_tk_add->createby->caption() ?><?php echo $m_karyawan_tk_add->createby->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $m_karyawan_tk_add->RightColumnClass ?>"><div <?php echo $m_karyawan_tk_add->createby->cellAttributes() ?>>
<span id="el_m_karyawan_tk_createby">
<?php
$onchange = $m_karyawan_tk_add->createby->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$m_karyawan_tk_add->createby->EditAttrs["onchange"] = "";
?>
<span id="as_x_createby">
	<input type="text" class="form-control" name="sv_x_createby" id="sv_x_createby" value="<?php echo RemoveHtml($m_karyawan_tk_add->createby->EditValue) ?>" size="30" maxlength="10" placeholder="<?php echo HtmlEncode($m_karyawan_tk_add->createby->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($m_karyawan_tk_add->createby->getPlaceHolder()) ?>"<?php echo $m_karyawan_tk_add->createby->editAttributes() ?>>
</span>
<input type="hidden" data-table="m_karyawan_tk" data-field="x_createby" data-value-separator="<?php echo $m_karyawan_tk_add->createby->displayValueSeparatorAttribute() ?>" name="x_createby" id="x_createby" value="<?php echo HtmlEncode($m_karyawan_tk_add->createby->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["fm_karyawan_tkadd"], function() {
	fm_karyawan_tkadd.createAutoSuggest({"id":"x_createby","forceSelect":false});
});
</script>
<?php echo $m_karyawan_tk_add->createby->Lookup->getParamTag($m_karyawan_tk_add, "p_x_createby") ?>
</span>
<?php echo $m_karyawan_tk_add->createby->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<input type="text" data-table="m_karyawan_tk" data-field="x_bulan" name="x_bulan" id="x_bulan" size="30" maxlength="10" placeholder="<?php echo HtmlEncode($m_karyawan_tk_add->bulan->getPlaceHolder()) ?>" value="<?php echo $m_karyawan_tk_add->bulan->EditValue ?>"<?php echo $m_karyawan_tk_add->bulan->editAttributes() ?>>
</span>
