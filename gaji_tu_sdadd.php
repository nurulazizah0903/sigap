...
loadjs.ready("head", function() {
	...
	fgaji_tu_sdadd.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
			<?php if ($gaji_tu_sd_add->by->Required) { ?>
				elm = this.getElements("x" + infix + "_by");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gaji_tu_sd_add->by->caption(), $gaji_tu_sd_add->by->RequiredErrorMessage)) ?>");
			<?php } ?>
	...
	fgaji_tu_sdadd.lists["x_by"] = <?php echo $gaji_tu_sd_add->by->Lookup->toClientList($gaji_tu_sd_add) ?>;
	fgaji_tu_sdadd.lists["x_by"].options = <?php echo JsonEncode($gaji_tu_sd_add->by->lookupOptions()) ?>;
