...
loadjs.ready("head", function() {
	...
	fgaji_karyawan_tkedit.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($gaji_karyawan_tk_edit->jp->Required) { ?>
				elm = this.getElements("x" + infix + "_jp");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $gaji_karyawan_tk_edit->jp->caption(), $gaji_karyawan_tk_edit->jp->RequiredErrorMessage)) ?>");
			<?php } ?>
				elm = this.getElements("x" + infix + "_jp");
				if (elm && !ew.checkInteger(elm.value))
					return this.onError(elm, "<?php echo JsEncode($gaji_karyawan_tk_edit->jp->errorMessage()) ?>");
...
<?php } ?>
<?php if ($gaji_karyawan_tk_edit->jp->Visible) { // jp ?>
	<div id="r_jp" class="form-group row">
		<label id="elh_gaji_karyawan_tk_jp" for="x_jp" class="<?php echo $gaji_karyawan_tk_edit->LeftColumnClass ?>"><?php echo $gaji_karyawan_tk_edit->jp->caption() ?><?php echo $gaji_karyawan_tk_edit->jp->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $gaji_karyawan_tk_edit->RightColumnClass ?>"><div <?php echo $gaji_karyawan_tk_edit->jp->cellAttributes() ?>>
<span id="el_gaji_karyawan_tk_jp">
<input type="text" data-table="gaji_karyawan_tk" data-field="x_jp" name="x_jp" id="x_jp" size="30" maxlength="11" placeholder="<?php echo HtmlEncode($gaji_karyawan_tk_edit->jp->getPlaceHolder()) ?>" value="<?php echo $gaji_karyawan_tk_edit->jp->EditValue ?>"<?php echo $gaji_karyawan_tk_edit->jp->editAttributes() ?>>
</span>
<?php echo $gaji_karyawan_tk_edit->jp->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
