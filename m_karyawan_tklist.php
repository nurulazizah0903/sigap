...
loadjs.ready("head", function() {
	...
	fm_karyawan_tklistsrch.validate = function(fobj) {
		...
		if (elm && !ew.checkInteger(elm.value))
			return this.onError(elm, "<?php echo JsEncode($m_karyawan_tk_list->bulan->errorMessage()) ?>");
...
<input type="text" data-table="m_karyawan_tk" data-field="x_bulan" name="x_bulan" id="x_bulan" size="30" maxlength="10" placeholder="<?php echo HtmlEncode($m_karyawan_tk_list->bulan->getPlaceHolder()) ?>" value="<?php echo $m_karyawan_tk_list->bulan->EditValue ?>"<?php echo $m_karyawan_tk_list->bulan->editAttributes() ?>>
</span>
	</div>
	<?php if ($m_karyawan_tk_list->SearchColumnCount % $m_karyawan_tk_list->SearchFieldsPerRow == 0) { ?>
</div>
...
?>
<?php if ($m_karyawan_tk_list->createby->Visible) { // createby ?>
	<?php if ($m_karyawan_tk_list->SortUrl($m_karyawan_tk_list->createby) == "") { ?>
		<th data-name="createby" class="<?php echo $m_karyawan_tk_list->createby->headerCellClass() ?>"><div id="elh_m_karyawan_tk_createby" class="m_karyawan_tk_createby"><div class="ew-table-header-caption"><?php echo $m_karyawan_tk_list->createby->caption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="createby" class="<?php echo $m_karyawan_tk_list->createby->headerCellClass() ?>"><div class="ew-pointer" onclick="ew.sort(event, '<?php echo $m_karyawan_tk_list->SortUrl($m_karyawan_tk_list->createby) ?>', 1);"><div id="elh_m_karyawan_tk_createby" class="m_karyawan_tk_createby">
			<div class="ew-table-header-btn"><span class="ew-table-header-caption"><?php echo $m_karyawan_tk_list->createby->caption() ?></span><span class="ew-table-header-sort"><?php if ($m_karyawan_tk_list->createby->getSort() == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($m_karyawan_tk_list->createby->getSort() == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($m_karyawan_tk_list->tahun->Visible) { // tahun ?>
...
?>
	<?php if ($m_karyawan_tk_list->createby->Visible) { // createby ?>
		<td data-name="createby" <?php echo $m_karyawan_tk_list->createby->cellAttributes() ?>>
<span id="el<?php echo $m_karyawan_tk_list->RowCount ?>_m_karyawan_tk_createby">
<span<?php echo $m_karyawan_tk_list->createby->viewAttributes() ?>><?php echo $m_karyawan_tk_list->createby->getViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($m_karyawan_tk_list->tahun->Visible) { // tahun ?>
